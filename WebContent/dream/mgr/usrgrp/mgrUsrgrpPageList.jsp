...
function initGrid()
{
    ...
    myGrid.attachEvent("onCheck",function(rowId, columnId, state){
    	...
    	for(var i=0, length= ids.length; i< length; i++) {
    		setValueById(myGrid, ids[i], "ISDELCHECK", state);
...
function goSave()
{
	...
	for(var i=0, length= ids.length; i< length; i++) {
		...
		rowObj['check'] = getValueById(myGrid, ids[i],'ISDELCHECK');
		rows.push(rowObj);
// 		menuMap[getValueById(myGrid, ids[i],'PGPAGEID')] = getValueById(myGrid, ids[i],'ISDELCHECK');
	}
	maUsrGrpAuthListForm.elements['maUsrGrpCommonDTO.jsonArray'].value = JSON.stringify(rows);
	
	maUsrGrpAuthListForm.strutsAction.value = '<%=MaUsrGrpAuthListAction.USRGRP_AUTHPAGE_LIST_SAVE%>';
	var url = contextPath + "/mgrUsrgrpPageList.do";
...
function goSearch()
{
    findGridList('Search', '<%=MaUsrGrpAuthListAction.USRGRP_AUTHPAGE_LIST_FIND%>');   
}
...
function goPmstd()
{
	...
	maUsrGrpAuthListForm.elements['maUsrGrpCommonDTO.pageId'].value = pageId;
		
    openLayerPopup("mgrUsrgrpBtnList", FormQueryString(maUsrGrpAuthListForm)); 
}
...
<html:form action="/mgrUsrgrpPageList.do">
