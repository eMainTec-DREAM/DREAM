...
function initGrid()
{
    ...
    myGrid.attachEvent("onBeforeSorting",function(ind,type,direction){
        ...
        return sortColumn("maPtBuyReqHdrDocLibList", this, maDocLibListForm, "DOCID", ind, direction);
    });
	//Inner Page
...
{
    var url = contextPath + "/maPtBuyReqHdrDocLibList.do";
...
function goOpen()
{
	goTabPage('maPtBuyReqHdrDocLibDetail');
}
function goOpenAction()
{
    ...
    openQuickTabPage(FormQueryString(maDocLibListForm),'maPtBuyReqHdrDocLibDetail');
}
/**
 * 생성
 */
function goCreate()
{
	createValidationCheck(myGrid, "maPtBuyReqHdrDocLibDetail" , "goCreateAction");
}
...
{
	goClose('maPtBuyReqHdrDocLibDetail', this);
	//goSearch();
	alertMessage1('<bean:message key="MESSAGE.MSG021"/>');
	
	if(typeof enhanceAfterDelete == "function") enhanceAfterDelete();
}
...
{
	maDocLibListForm.elements['maDocLibCommonDTO.docId'].value = "";
    excelServerAction("maPtBuyReqHdrDocLibList", maDocLibListForm );  
}
...
<html:form action="/maPtBuyReqHdrDocLibList.do">
...
<html:hidden property="maDocLibCommonDTO.description" />
	...
	<c:set var="filePath" value="enhance/${compName}/part/pur/buy/maPtBuyReqHdrDocLibList_${compNo}.jsp" />
	<c:if test="${udf:isExist(filePath)}">
		<c:import charEncoding="UTF-8" url="/enhance/${compName}/part/pur/buy/maPtBuyReqHdrDocLibList_${compNo}.jsp"></c:import>
	</c:if>
</html:form> 
</body>
</html>