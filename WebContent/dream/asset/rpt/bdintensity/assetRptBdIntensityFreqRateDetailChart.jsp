...
<head>
<!-- 고장도수율(월별) -->
<title><bean:message key='MENU.BDINTENSITY'/></title>
...
function initFusionChart(_data) {
	...
	for(var i = contentsCnt-1; i>=0; i--)
	{
		...
		myObj.data[i].value = myObj.data[i].BDFREQRATE;
		delete myObj.data[i].BDFREQRATE;
	}
    ...
    var revenueChart = new FusionCharts({
        ...
        dataSource: {
            ...
            "chart": {
                ...
                , "yAxisName": "<bean:message key='LABEL.bdFreqRate'/>"
                , "baseFontSize" : "14"
...
function findChart(idx,plant,line){
	var url = contextPath + "/assetRptBdIntensityFreqRateDetailChart.do";
	var form = document.assetRptBdIntensityDetailListForm;	
	form.strutsAction.value = '<%=AssetRptBdIntensityDetailListAction.DETAIL_FREQ_FIND %>';
...
<html:form action="/assetRptBdIntensityFreqRateDetailChart.do">
