...
SELECT 
    ...
    , x.maker 																							maker
    , x.model_no 																						model
    , SFACODE_TO_DESC(x.eq_status,'EQ_STATUS','SYS','','{loginUser.langId}')  										eqStatus 
    ...
    , SFACODE_TO_DESC(x.pmi_action_type,'PMI_ACTION_TYPE','SYS','','{loginUser.langId}')  							siaGrade
	, x.tag_no																							tagNo
    , x.serial_no 																						serialNo
	, x.as_vendor 																						asName
    , x.as_name  																						asManager
    , x.as_phone  																						asNum
    , SFACODE_TO_DESC(x.eq_grade,'EQ_GRADE','SYS','','{loginUser.langId}')  										eqGrade        
    , x.ord_no																							ordNo
...
SELECT 
      ...
      , nvl(REPLACE(REPLACE(y.perform,chr(10),''),chr(13),'  '), '') 								perform
