...
작업결과(고장작업-교체) 투입자재
author  kim21017
version $Id: maWoResultBmRplPartList.jsp,v 1.1 2015/12/03 01:45:27 kim21017 Exp $
since   1.0
===========================================================================--%>
...
function loadPage() 
{
	...
	maWoResultPartListForm.elements['maWoResultPartDetailDTO.equipId'].value 
	= maWoResultPartListForm.elements['maWoResultMstrDetailDTO.equipId'].value;
...
function initGrid()
{
	...
	myGrid.attachEvent("onRowSelect",function(rowId, columnId){
		goTabPage("maWoResultBmRplPartDetail");
	});
...
function goOpenAction()
{
	var selectedId=myGrid.getSelectedRowId();
 	...
 	openQuickTabPage(FormQueryString(maWoResultPartListForm), 'maWoResultBmRplPartDetail'); 
 ...
 function goCreate()
 {
  	createValidationCheck(myGrid, "maWoResultBmRplPartDetail" , "goCreateAction");
 }
...
function afterDelete(){
	goClose('maWoResultBmRplPartDetail',this);
	//goSearch();
...
function setAcLovValue(rtnArr, code)
{
	if(code == 'maWoResultPartDetailDTO.multiDesc') {
	    ...
	    maWoResultPartListForm.elements['maWoResultPartDetailDTO.useQty'].value = '1';
	  	...
	  	//부위
		maWoResultPartListForm.elements['maWoResultPartDetailDTO.eqAsmbId'].value 
		= parent.M$('maWoResultMstrDetailDTO.eqAsmbId').value;
...
<html:hidden property="maWoResultPartDetailDTO.eqAsmbDesc"/>
...
<html:hidden property="maWoResultPartDetailDTO.equipDesc"/>
    ...
    <div class="article_box" id="listBox">
 	...
 	<c:set var="filePath" value="enhance/${compName}/work/list/bm/maWoResultBmRplPartList_${compNo}.jsp" />
	<c:if test="${udf:isExist(filePath)}">
		<c:import charEncoding="UTF-8" url="/enhance/${compName}/work/list/bm/maWoResultBmRplPartList_${compNo}.jsp"></c:import>
	</c:if>
