...
검교정상세 측정값목록
author  kim21017
version $Id: workListPrsCavalList.jsp,v 1.1 2015/12/03 01:45:27 kim21017 Exp $
since   1.0
===========================================================================--%>
...
function initGrid()
{
	...
	myGrid.setHeader("<bean:message key='LABEL.seqNo'/>,<bean:message key='LABEL.ISDELCHECK'/>,<bean:message key='LABEL.calibPointType'/>,<bean:message key='LABEL.calibPoint'/>,<bean:message key='LABEL.allowValue'/>,<bean:message key='LABEL.asFound'/>,#cspan,#cspan,<bean:message key='LABEL.asLeft'/>,#cspan,#cspan,ID,ID");
	...
	myGrid.attachEvent("onXLE",function(grdObj,count){setCounter(myGrid,"gridbox", true)}); 
	myGrid.attachEvent("onRowSelect",function(rowId, columnId){
		goTabPage("workListPrsCavalDetail");
	});
	myGrid.init();
}
...
function findGridList(sheetAction)
{
	...
	var url = contextPath + "/workListPrsCavalList.do";
 ...
 */
 ...
 function goTabPageAction(pageId, selectedId)
 {
	...
	goCommonTabPage(workListGnlCavalListForm, '<%=WorkListGnlCavalDetailAction.WORK_LIST_GNL_CAVAL_DETAIL_INIT%>', pageId);
 }
 ...
 function goOpenAction()
 {
     ...
     openQuickTabPage(FormQueryString(workListGnlCavalListForm), 'workListPrsCavalDetail'); 
 }
 ...
 function goCreate()
 {
  	createValidationCheck(myGrid, "workListPrsCavalDetail" , "goCreateAction");
 }
...
function afterDelete(){
	goClose('workListPrsCavalDetail',this);
...
function goCopycalibstdvalue(){
	...
	var actionUrl = contextPath + "/workListPrsCavalList.do";
...
<html:form action="/workListPrsCavalList.do">
