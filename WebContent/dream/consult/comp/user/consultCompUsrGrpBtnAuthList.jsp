...
function goSave()
{
	...
	for(var i=0, length= ids.length; i< length; i++) {
		...
		menuMap[getValueById(myGrid, ids[i],'pgbtnId')] = getValueById(myGrid, ids[i],'AUTHCHECK');
	}

	consultCompUsrGrpAuthListForm.elements['consultCompUsrGrpCommonDTO.jsonArray'].value = JSON.stringify(menuMap);
	
	consultCompUsrGrpAuthListForm.strutsAction.value = '<%=ConsultCompUsrGrpAuthListAction.USRGRP_AUTHPBTN_LIST_SAVE%>';
	var url = contextPath + "/consultCompUsrGrpBtnAuthList.do";
...
function findGridList(actionType,sheetAction)
{
    ...
    doGridAction(actionType, myGrid, "gridbox", url, FormQueryString(consultCompUsrGrpAuthListForm), "PAGEID");
...
function goSearch()
{
    findGridList('Search', '<%=ConsultCompUsrGrpAuthListAction.USRGRP_AUTHBTN_LIST_FIND%>');   
}
...
function goExcel()
{
	excelServerAction("consultCompUsrGrpBtnAuthList", consultCompUsrGrpAuthListForm);
}
...
<html:form action="/consultCompUsrGrpBtnAuthList.do">
