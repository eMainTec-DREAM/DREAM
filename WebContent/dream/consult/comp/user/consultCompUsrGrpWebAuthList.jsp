...
function goSave()
{
	...
	//var ids = myGrid.getCheckedRows(getCoumnIdx(myGrid, 'AUTHCHECK'));

	if(ids == "") return;

	ids = ids.split(",");
	for(var i=0, length= ids.length; i< length; i++) {
		console.log(getValueById(myGrid, ids[i],'AUTHCHECK'));
	...
	var url = contextPath + "/consultCompUsrGrpWebAuthList.do";
...
{
    var url = contextPath + "/consultCompUsrGrpWebAuthList.do";
    consultCompUsrGrpAuthListForm.elements['consultCompUsrGrpCommonDTO.filterServiceType'].value = "WEB";
...
{
    //excelAction(myGrid);
    excelServerAction("consultCompUsrGrpWebAuthList", consultCompUsrGrpAuthListForm);
}
...
<html:form action="/consultCompUsrGrpWebAuthList.do">
