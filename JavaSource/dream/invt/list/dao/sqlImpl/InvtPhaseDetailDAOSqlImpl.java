 ...
 */
public class InvtPhaseDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements InvtPhaseDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public InvtPhaseDetailDTO findDetail(InvtCommonDTO invtCommonDTO, User user)
    {
        ...
        query.append("       ,dbo.SFACODE_TO_DESC(invt_proc_ltype,'INVT_PROC_LTYPE','USR','"+user.getCompNo()+"','"+user.getLangId()+"') invtProcLtypeDesc      ");
        query.append("       ,dbo.SFACODE_TO_DESC(invt_proc_stype,'INVT_PROC_STYPE','USR','"+user.getCompNo()+"','"+user.getLangId()+"') invtProcStypeDesc      ");
        query.append("       ,ref_depart refDepart                              ");
        query.append("       ,ref_doc refDoc                                    ");
        query.append("       ,invtphase_status invtphaseStatus                  ");
        query.append("       ,dbo.SFACODE_TO_DESC(invtphase_status,'INVTPHASE_STATUS','SYS','','ko') invtphaseStatusDesc        ");
...
//              ,invtCommonDTO.getRcmlistId()
	...
	public String copyDetail(String oldInvtId, String newInvtId, String oldKeyId, String newKeyId, User user) throws Exception 
	{
    	...
    	{	// 전체 복사인 경우
    		query.append("   , NEXT VALUE FOR sqainvtphase_id   							");
    	}
