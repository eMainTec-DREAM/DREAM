 ...
 */
public class InvtListDAOSqlImpl extends BaseJdbcDaoSupportSql implements InvtListDAO
{
    /**
     * grid find
     * @author  kim21017
    ...
    public List findList(InvtCommonDTO invtCommonDTO, User user)
    {
        ...
        query.append("        ,(SELECT dbo.SFACODE_TO_DESC(c.invt_proc_ltype,'INVT_PROC_LTYPE','USR','"+user.getCompNo()+"','"+user.getLangId()+"')       ");
        query.append("          FROM (SELECT invt_proc_ltype, ");
        query.append("                       invtlist_id,   ");
        query.append("                       comp_no,   ");
        query.append("                       ROW_NUMBER() OVER(ORDER BY ord_no DESC) ROWNUM ");
        ...
        query.append("       ,(SELECT CONVERT(VARCHAR, round(sum((CASE WHEN a.invtphase_status='C' THEN 1 ELSE 0 END))/CONVERT(FLOAT,sum(1))*100,2))+'%'		");
        query.append("         FROM tainvtphase a												");
        query.append("         WHERE  1=1														");
        query.append("         AND a.invtlist_id = x.invtlist_id 								");
        query.append("         AND a.comp_no = x.comp_no										");
        query.append("         GROUP BY a.comp_no , a.invtlist_id) actualRate					");
        query.append("       ,(SELECT top 1 b.emp_name                                                ");
        ...
        query.append("       ,dbo.SFACODE_TO_DESC(x.invt_kind,'INVT_KIND','SYS','','"+user.getLangId()+"') INVTKINDDESC	");
    ...
    {        
        ...
        query.getCodeLikeQuery("x.invt_categ", "dbo.SFACODE_TO_DESC(invt_categ,'INVT_CATEG','SYS','','"+user.getLangId()+"')",invtCommonDTO.getInvtCateg(), invtCommonDTO.getInvtCategDesc());
        ...
        //공장코드
        query.getCodeLikeQuery("x.plant", "(SELECT a.description FROM  TAPLANT a WHERE a.comp_no = '"+user.getLangId()+"' AND a.plant = x.plant )", 
