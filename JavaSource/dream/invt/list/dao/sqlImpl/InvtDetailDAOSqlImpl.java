 ...
 */
public class InvtDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements InvtDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public InvtDetailDTO findDetail(InvtCommonDTO invtCommonDTO, User user)
    {
        ...
        //query.append("       ,(SELECT a.woreq_id FROM TAWOREQRES a WHERE x.comp_no = a.comp_no AND x.invtlist_id = a.invtlist_id) 		 WOREQID	");
     ...
     * @return
     */
    public int updateDetail(InvtDetailDTO invtDetailDTO, User user)
    {
        QuerySqlBuffer query = new QuerySqlBuffer();
    ...
    public int insertPhase(InvtDetailDTO invtDetailDTO, User user) {
		...
		query.append("  SELECT ? 			, NEXT VALUE FOR SQAINVTPHASE_ID 	, ?  				");
		query.append("         , y.invtprcph_id 	, y.ord_no 					, y.invt_proc_ltype	");
...
//                , user.getCompNo()
	...
	public int cancelInvtStatus(InvtDetailDTO invtDetailDTO, User user) throws IOException {
        ...
        query.append("UPDATE TAINVTLIST SET                     ");
        query.append("       invtlist_status          = 'P'     ");
        query.append("WHERE  invtlist_id              = ?       ");
        query.append("  AND  comp_no                  = ?       ");
	...
	public int cancelInvtPhaseStatus(InvtDetailDTO invtDetailDTO, User user) throws IOException {
        ...
        query.append("WHERE  comp_no				  = ?						");
        query.append("  AND  invtphase_id = (SELECT TOP 1 invtphase_id			");
        query.append("                                    FROM TAINVTPHASE		");
        query.append("                                    WHERE invtlist_id = ?	");
        query.append("                                    AND comp_no = ?		");
