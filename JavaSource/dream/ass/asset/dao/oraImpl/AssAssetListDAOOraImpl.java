 ...
 * @author youngjoo38
 * @version $Id: AssAssetListDAOOraImpl.java,v 1.0 2017/08/22 15:19:40 youngjoo38 Exp $
 * @since 1.0
 * 
 ...
 */
public class AssAssetListDAOOraImpl  extends BaseJdbcDaoSupportOra implements AssAssetListDAO
{

    /**
     * grid find
     * @author  youngjoo38
     * @version $Id: assAssetListDAO.java,v 1.0 2017/08/22 15:20:12 youngjoo38 Exp $
     * @since   1.0
     * 
    ...
    public List findList(AssAssetCommonDTO assAssetCommonDTO, User user) throws Exception
    {
        ...
        query.getOrderByQuery("x.eqasslist_id DESC", assAssetCommonDTO.getOrderBy(), assAssetCommonDTO.getDirection());
    ...
    {        
        ...
        // 설비 (ID/DESC)
        if(!"".equals(assAssetCommonDTO.getFilterEquipDesc()) && !"".equals(assAssetCommonDTO.getFilterEquipId()))
        {
            query.getAndQuery("x.equip_id", assAssetCommonDTO.getFilterEquipId());
        }
        else if(!"".equals(assAssetCommonDTO.getFilterEquipDesc()))
        {
        ...
        //설비고장(분) 
        query.getInequalityQuery("(SELECT SUM(a.eval_value) FROM TAEQASSPVAL a WHERE a.comp_no = x.comp_no AND a.eqasslist_id = x.eqasslist_id GROUP BY a.comp_no, a.eqasslist_id)", assAssetCommonDTO.getConOperDesc(), assAssetCommonDTO.getAssScore());

        // 설비번호
        query.getLikeQuery("y.item_no", assAssetCommonDTO.getFilterEquipNo());
        
        //설비사용부서
        query.getDeptLevelQuery("(SELECT a.usage_dept FROM TAEQUIPMENT a WHERE a.comp_no = y.comp_no AND a.equip_id =  y.p_equip_id)", assAssetCommonDTO.getFilterParUsageDeptId(), assAssetCommonDTO.getFilterParUsageDeptDesc(), user.getCompNo());
