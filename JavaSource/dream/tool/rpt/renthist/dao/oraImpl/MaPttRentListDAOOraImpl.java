package dream.tool.rpt.renthist.dao.oraImpl;
 ...
 */
public class MaPttRentListDAOOraImpl extends BaseJdbcDaoSupportOra implements MaPttRentListDAO
{
    /**
     * grid find
     * @author  ssong
    ...
    public List findList(MaPttRentCommonDTO maPttRentCommonDTO)
    {
        ...
        query.append("(SELECT NVL(SUM(z.rtn_qty),0) FROM TAPTRTNLIST z WHERE z.comp_no='"+compNo+"' AND z.ptrtn_status='W' AND z.wcode_id=x.wcode_id AND x.part_id=z.part_id AND x.rec_by=z.rec_by)returnQty,");
    ...
    {        
     ...
     // 창고명
        ...
        query.getLikeQuery("full_desc", maPttRentCommonDTO.getFilterPartNameSize());
        // 품명/규격
    ...
    public int reqHdrPtRtn(MaPttRentListForm maPttRentListForm, String ptrentlistId, User user)
    {
    	...
    	query.append("  remark                                                  ");
    	query.append(")  SELECT ");
    	query.append("  comp_no,    SQAPTRTNLIST_ID.NEXTVAL,   wcode_id,   part_grade,");
    	query.append("  TO_CHAR(SYSDATE, 'YYYYMMDD'), part_id,   rent_qty,    'W',");
