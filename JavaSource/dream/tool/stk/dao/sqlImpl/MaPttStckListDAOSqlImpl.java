 ...
 */
public class MaPttStckListDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaPttStckListDAO
{
    /**
     * grid find
     * @author  ssong
    ...
    public List findPtStckList(MaPttStckCommonDTO maPttStckCommonDTO)
    {
        ...
        query.append("       ,ISNULL(a.stock_qty,0) AS stockQty                          ");
        ...
        query.append("       ,ISNULL(a.rent_qty,0) AS rentQty                    		  ");
        query.append("       ,CASE WHEN ISNULL(a.min_safty_qty,0) - (ISNULL(a.tot_stock_qty,0) - ISNULL(a.rent_qty,0)) < 0 THEN 0    ");
        query.append("           ELSE ISNULL(a.min_safty_qty,0) - (ISNULL(a.tot_stock_qty,0) - ISNULL(a.rent_qty,0)) END AS underSaftyQty    ");
        ...
        query.append("           ,ISNULL(c.description,'')+','+ISNULL(c.pt_size,'') as partNameSize        ");
        ...
        query.append("           ,SUM(a.stock_qty)+(SELECT ISNULL(SUM(z1.rent_qty),0) FROM TAPTRENT_STOCK z1 WHERE z1.comp_no=a.comp_no AND z1.wcode_id=a.wcode_id AND a.part_id=z1.part_id) AS tot_stock_qty  ");
        ...
        query.append("GROUP BY a.comp_no,a.wcode_id,b.wname,a.part_id,c.part_no,ISNULL(c.description,'')+','+ISNULL(c.pt_size,'')");
        query.append(") a															");
        query.append("WHERE 1=1														");
        if("Y".equals(maPttStckCommonDTO.getFilterSaftyYN())){
        	query.append("AND (ISNULL(a.min_safty_qty,0) - (ISNULL(a.tot_stock_qty,0) - ISNULL(a.rent_qty,0))) > 0	");
        }else if("N".equals(maPttStckCommonDTO.getFilterSaftyYN())){
        	query.append("AND (ISNULL(a.min_safty_qty,0) - (ISNULL(a.tot_stock_qty,0) - ISNULL(a.rent_qty,0))) <= 0	");
        }
        if(!"".equals(maPttStckCommonDTO.getFilterQtyCnt())){
        	query.append("AND ISNULL(a.tot_stock_qty,0) >= "+maPttStckCommonDTO.getFilterQtyCnt()+"");
        }
        query.getOrderByQuery("a.part_id","a.part_no DESC", maPttStckCommonDTO.getOrderBy(), maPttStckCommonDTO.getDirection());
    ...
    public int reqHdrPtStck(MaPttStckListForm maPttStckListForm, String wcodeId, String partId,User user)
    {
    	...
    	query.append("	)	VALUES										");
    	query.append("	(?,				?,				?,				");
    	query.append("	 ?,(SELECT description+'...' FROM TAPARTS WHERE comp_no = '"+user.getCompNo()+"' AND part_id = '"+partId+"'),	CONVERT(VARCHAR, GETDATE(), 112),				");
    	query.append("	 ?,            ?,         ?					");
    	query.append("	)												");
