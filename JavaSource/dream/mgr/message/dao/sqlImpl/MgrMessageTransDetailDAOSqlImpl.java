 ...
 */
public class MgrMessageTransDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MgrMessageTransDetailDAO
{
    ...
    public MgrMessageTransDetailDTO findDetail(MgrMessageTransCommonDTO mgrMessageTransCommonDTO, User user) throws Exception
    {
        ...
        query.append("      ,dbo.SFACODE_TO_DESC(x.message_status,'MESSAGE_STATUS','SYS','','"+user.getLangId()+"')	AS msgStatusDesc	");
        ...
        MgrMessageTransDetailDTO mgrMessageTransDetailDTO = 
        		(MgrMessageTransDetailDTO)getJdbcTemplate().query(query.toString(), getObject(objects), new MwareExtractor(new MgrMessageTransDetailDTO()));
    ...
    public List findErrorLog(String errorLogId, User user) throws Exception
    {
        ...
        query.append("    , CONVERT(varchar,x.err_date,20)              AS ERROR_DATE   ");
        query.append("    , x.comp_no +' / '+x.user_no                  AS COMP_USER    ");
        query.append("    , x.req_url                                   AS URL          ");
        query.append("    , REPLACE(REPLACE(CONVERT(nvarchar(MAX), x.stacktrace_clob), char(13), ''), char(10), '<br>')   AS ERROR_LOG    ");
    ...
    public List getDataREQ10(String objectId, User user) throws Exception
    {
        ...
        query.append("    ,dbo.SFACODE_TO_DESC(a.woreq_status,'WOREQ_STATUS','SYS','',?)                                                  AS WOREQ_STATUS                    ");
        ...
        query.append("    ,isnull((select aa.item_no from taequipment aa where a.comp_no = aa.comp_no and a.equip_id = aa.equip_id ),'-') AS ITEM_NO                        ");
        query.append("    ,isnull(a.mo_desc,'-')                                                                                          AS MO_DESC                        ");
        query.append("    ,a.comp_no                                                                                                      AS COMP_NO                        ");
    ...
    public List getDataRQC10(String objectId, User user) throws Exception
    {
        ...
        query.append("    ,isnull((select aa.full_desc from taeqloc aa where a.comp_no = aa.comp_no and a.eqloc_id = aa.eqloc_id ),'-')                 as LOC_NAME           ");
        query.append("    ,replace(replace(isnull(a.request,'-'), char(10), '<br>' ),' ', '&nbsp;')                                                      as REQUEST           ");
        query.append("    ,replace(replace(isnull(a.review,'-'), char(10), '<br>' ),' ', '&nbsp;')                                                      as REVIEW           ");
        query.append("from taworeq a                                                                                                                                        ");
        query.append("where a.comp_no = ?                                                                                                                                   ");
        query.append("    and a.woreq_id = ?                                                                                                                                ");
    ...
    public List getDataWRK10(String objectId, User user) throws Exception
    {
        ...
        query.append("    ,dbo.SFACODE_TO_DESC(a.wo_status,'WO_STATUS','SYS','',?)                                                       as WO_STATUS                      ");
        ...
        query.append("    ,isnull((SELECT aa.full_desc FROM TAEQASMB aa WHERE a.comp_no = aa.comp_no ANd a.eqasmb_id = aa.eqasmb_id),'-')   AS EQASMB                         ");
    ...
    public List getDataISS10(String objectId, User user) throws Exception
    {
        ...
        query.append("    ,dbo.SFACODE_TO_DESC(a.ptiss_status,'PTISS_STATUS','SYS','',?)                                                AS PTISS_STATUS     ");
        query.append("    ,substring(a.issue_date,1,4) + '-' +substring(a.issue_date,5,2) + '-' +substring(a.issue_date,7,2)            AS ISSUE_DATE       ");
        query.append("    ,dbo.SFACODE_TO_DESC(a.ptiss_type,'PTISS_TYPE','SYS','',?)                                                    AS PTISS_TYPE       ");
        query.append("    ,isnull((SELECT wname FROM TAWAREHOUSE aa                                                                                         ");
        ...
        query.append("    ,(SELECT description+', '+isnull(pt_size,'') FROM TAPARTS WHERE comp_no=a.comp_no AND part_id=a.part_id)      AS PART_DESC        ");
    ...
    public List getDataQNA20(String objectId, User user) throws Exception
    {
        ...
        query.append("     ,dbo.SFACODE_TO_DESC(y.helpdesk_status,'HELPDESK_STATUS','SYS','',?)                                         AS HELPDESK_STATUS  ");
        ...
        query.append("            then  substring(y.req_date,1,4) + '-' + substring(y.req_date,5,2) + '-' + substring(y.req_date,7,2)                       ");
        query.append("            else '-' end                                                                                          AS REQ_DATE         ");
        query.append("     ,y.view_name                                                                                                 AS VIEW_BY          ");
        query.append("     ,case when y.view_date is not null                                                                                               ");
        query.append("            then  substring(y.view_date,1,4) + '-' + substring(y.view_date,5,2) + '-' + substring(y.view_date,7,2)                    ");
        query.append("            else '-' end                                                                                          AS VIEW_DATE        ");
        query.append("     ,replace(replace(isnull(y.request,'-'), char(10), '<br>' ),' ', '&nbsp;')                                    AS REQUEST          ");
        query.append("     ,replace(replace(isnull(y.perform,'-'), char(10), '<br>' ),' ', '&nbsp;')                                    AS PERFORM          ");
    ...
    public List getDataPPR10(String objectId, User user) throws Exception
    {
        ...
        query.append("      , dbo.SFACODE_TO_DESC(x.ptpnlist_status, 'PTPNLIST_STATUS', 'SYS', '',?)       AS PTPNLISTSTATUS ");
        query.append("      , ( SELECT                                                                                       ");
        ...
        query.append("      , isnull(x.usage,'-')                                                          AS USAGE          ");
        query.append("      , replace(replace(isnull(x.remark,'-'), char(10), '<br>' ),' ', '&nbsp;')      AS REMARK         ");
        query.append("      , x.comp_no                                                                    AS COMP_NO        ");
        query.append("      , x.rec_by                                                                     AS REC_BY         ");
    ...
    public List getSubListPRI10(String objectId, User user) throws Exception
    {
        ...
        query.append("     , x.description +',' + ISNULL(x.PT_SIZE,'') PARTDESC        ");
