 ...
 * @version $Id:$
 * @since   1.0
 * @spring.bean id="mgrLangSyscodeDAOTarget"
 * @spring.txbn id="mgrLangSyscodeDAO"
 * @spring.property name="dataSource" ref="dataSource"
 */
public class MgrLangSyscodeDAOSqlImpl extends BaseJdbcDaoSupportSql implements MgrLangSyscodeDAO
{
	...
	private String getColums(MaResCommonDTO maResCommonDTO, User user) {
    	...
    	query.append("    WHERE a.list_type = x.list_type) 	LISTTYPEDESC		");
    	query.append(" , x.cdsysd_no 						CDSYSDNO			");
    	query.append(" , (SELECT a.key_name 									");
    	query.append("      FROM TALANG a 										");
    	query.append("    WHERE a.key_no = x.key_no AND a.key_type = x.key_type AND a.lang = '"+user.getLangId()+"') 	CDSYSDDESC		");
	...
	private String getOrderBy(MaResCommonDTO maResCommonDTO, User user) {
		...
		query.getOrderByQuery("x.cdsysd_id","x.cdsysd_id", maResCommonDTO.getOrderBy(), maResCommonDTO.getDirection());		
    ...
    {        
		...
		query.append("					AND a.key_type = 'CODESET')		");
