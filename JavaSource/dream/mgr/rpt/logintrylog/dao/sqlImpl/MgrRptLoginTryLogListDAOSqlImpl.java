package dream.mgr.rpt.logintrylog.dao.sqlImpl;
 ...
 */
public class MgrRptLoginTryLogListDAOSqlImpl  extends BaseJdbcDaoSupportSql implements MgrRptLoginTryLogListDAO
{
    ...
    public List findList(MgrRptLoginTryLogCommonDTO mgrRptLoginTryLogCommonDTO, User user) throws Exception
    {
        ...
        query.append("    , dbo.SFACODE_TO_DESC(x.login_terminal, 'SERVICE_TYPE', 'SYS', x.comp_no,'"+user.getLangId()+"') 	tmlTypeDesc	");
        query.append("    , x.terminal_no 		tmlNo		");
        query.append("    , x.terminal_ver    	tmlVerDesc	");
        query.append("    ,									");
        query.getTime("x.login_time", "loginTime");
        query.append("    , dbo.SFACODE_TO_DESC(x.is_success, 'IS_USE', 'SYS', x.comp_no,'"+user.getLangId()+"') 	isSuccessDesc	");
        query.append("FROM TALOGINTRYLOG x 					");
        query.append("WHERE 1=1								");
        query.append(this.getWhere(mgrRptLoginTryLogCommonDTO, user));
        query.getOrderByQuery("x.LOGINTRYLOG_ID","x.login_date DESC, x.login_time DESC", mgrRptLoginTryLogCommonDTO.getOrderBy(), mgrRptLoginTryLogCommonDTO.getDirection());
     ...
     * Filter 조건
     * @author  youngjoo38
     * @version $Id: mgrRptLoginTryLogListDAO.java,v 1.0 2018/08/23 15:19:40 youngjoo38 Exp $
     * @since   1.0
     * 
     * @param mgrRptLoginTryLogCommonDTO
     * @return
