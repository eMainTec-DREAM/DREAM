 ...
 */
public class MaUserRptOrdDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaUserRptOrdDetailDAO
{
    /**
     * detail find
     * @author ssong
	...
	public MaUserRptOrdDetailDTO findDetail(MaUserRptCommonDTO maUserRptCommonDTO, User loginUser)
    {
        ...
        query.append("       dbo.SFACODE_TO_DESC(sort_type,'SORT_TYPE','SYS','','"+loginUser.getLangId()+"') sortTypeDesc,");
        query.append("       ISNULL(use_yn,'N') useYn,						");
        query.append("       ISNULL(step_num, (SELECT ISNULL(MAX(a.step_num)+1,1) FROM TAUSRRPTORD a WHERE a.usrrptlist_id = "+maUserRptCommonDTO.getUsrrptlistId()+")) stepNum,		");
