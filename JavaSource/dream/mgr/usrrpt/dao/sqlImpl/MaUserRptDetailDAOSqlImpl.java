 ...
 */
public class MaUserRptDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaUserRptDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public MaUserRptDetailDTO findDetail(String usrrpt_id, String lang)
    {
        ...
        query.append("SELECT                                                    	");
      //  query.append("       SFACODE_TO_DESC(x.usrrpt_type,'USRRPT_TYPE','SYS','','"+lang+"')    usrrptTypeDesc,    	");
	...
	public List<Map> findTableList(MaUserRptCommonDTO maUserRptCommonDTO, User user) {
        ...
        query.append("       table_name+CONVERT(varchar(10),usrrpttab_id,10)               as \"key\",			"); //TAPAGE2
	...
	public List<Map> findColList(MaUserRptCommonDTO maUserRptCommonDTO, User user) {
        ...
        query.append("       table_id tableId,					");
        query.append("       table_name+CONVERT(varchar(10),usrrpttab_id,10) as \"key\",	 	");
	...
	public List<Map> findParamList(MaUserRptCommonDTO maUserRptCommonDTO, User user) {
        ...
        query.append("       dbo.SFACODE_TO_DESC(whr_con_operator,'TAB_CON_OPERATOR','SYS','','"+user.getLangId()+"') whrConOperatorDesc,          	");
	...
	public List<Map> findJoinList(MaUserRptCommonDTO maUserRptCommonDTO, User user) {
        ...
        query.append("       dbo.SFACODE_TO_DESC(tab_con_operator,'TAB_CON_OPERATOR','SYS','','"+user.getLangId()+"') tabConOperatorDesc,          	");
        query.append("       (SELECT a.table_name FROM tausrrpttab a WHERE a.usrrpttab_id = x.lusrrpttab_id)+CONVERT(varchar(10),lusrrpttab_id,10) lkey,		");
        query.append("       (SELECT a.table_name FROM tausrrpttab a WHERE a.usrrpttab_id = x.rusrrpttab_id)+CONVERT(varchar(10),rusrrpttab_id,10) rkey,   	");
        query.append("       usrrpttab_id  usrrpttabId,    					");
        query.append("       usrrptlist_id usrrptlistId,					");
