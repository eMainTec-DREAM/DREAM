 ...
 */
public class ConsultProgramUploadDataDAOSqlImpl extends BaseJdbcDaoSupportSql implements ConsultProgramUploadDataDAO
{
    /**
     * grid find
     * @author  youngjoo38
    ...
    public List findList(ConsultProgramUploadDataDTO consultProgramUploadDataDTO, User user)
    {
        ...
        query.append("  , dbo.SFACODE_TO_DESC(x.excellist_status,'EXCELLIST_STATUS','SYS','','"+user.getLangId()+"') AS EXCELLISTSTATUS        ");
        query.append("  , x.remark              AS REMARK           ");
        query.append("FROM TAEXCELTAB x                             ");
        query.append("WHERE 1=1                                     ");
        query.append(this.getWhere(consultProgramUploadDataDTO, user));
        query.getOrderByQuery("x.exceltab_id","x.exceltab_no", consultProgramUploadDataDTO.getOrderBy(), consultProgramUploadDataDTO.getDirection());
     ...
     * @since   1.0
     * 
     * @param consultProgramUploadDataDTOList
     * @return
     */
    ...
    public int insertDetail(ConsultProgramUploadDataDTO consultProgramUploadDataDTO, User user)
    {
        ...
        query.append("    , ?                                  ");
        query.append("    , ?              , ?                 ");
        query.append("    ,CONVERT(CHAR(8),GETDATE(),112)    ,?   ");
        query.append("    , ?              , ?                 ");
        query.append("    )                                    ");
        ...
        resultValue = getJdbcTemplate().update(query.toString(), getObject(objects));
    ...
    public int updateDetail(ConsultProgramUploadDataDTO consultProgramUploadDataDTO, User user)
    {
        ...
        query.append("  upload_pgm_name      = ?,   ");
        query.append("  remark               = ?,   ");
        query.append("  upd_date             = CONVERT(CHAR(8),GETDATE(),112),   ");
        query.append("  upd_by               = ?    ");
    ...
    public String checkTableExist(ConsultProgramUploadDataDTO consultProgramUploadDataDTO, User user)
    {
        ...
        query.append("FROM INFORMATION_SCHEMA.TABLES x        ");
    ...
    public int completeDetail(ConsultProgramUploadDataDTO consultProgramUploadDataDTO, User user)
    {
        ...
        for (int i = 0; i<mapList.size(); i++)
        {
            ...
            query.append(") VALUES (                            ");
            query.append("  ?              , next value for sqaexcelcolumn_id      ");
            query.append(", ?              , ''                 ");
            query.append(", ?              , ?                  ");
        ...
        colCnt = Integer.parseInt(QuerySqlBuffer.listToString(getJdbcTemplate().queryForList(query.toString(),cntObjects)));
        ...
        if(colCnt>=1)
        {
            ...
            for(int i=1; i<=colCnt; i++)
            {
                ...
                query.setClear();
                query.append("SELECT                                ");
                query.append("    a.excelColId , a.excelTabId           ");
                query.append("  , a.ordNo      , a.excelColName         ");
                query.append("  , a.isKeyId    , a.tableColName         ");
                query.append("  , a.excelColComments , a.tableColTypeId ");
                query.append("  , a.tableColType        , a.tableColSize      ");
                query.append("FROM (                                                ");
                    query.append("SELECT                                            ");
                    query.append("  ROW_NUMBER() OVER(ORDER BY x.ord_no) AS rno                                   ");
                    ...
                    query.append("  , dbo.SFACODE_TO_DESC(x.table_col_type,'DATA_TYPE','SYS','','"+user.getLangId()+"') AS tableColType ");
