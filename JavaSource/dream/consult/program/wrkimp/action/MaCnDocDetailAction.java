 ...
 * @since 1.0
 * @struts:action path="/maWrkImpCnDocDetail" name="maDocLibDetailForm"
 *                input="/dream/consult/program/wrkimp/maWrkImpCnDocDetail.jsp" scope="request"
 *                validate="false"
 */
public class MaCnDocDetailAction extends ConsultAuthAction
{
            ...
            throws Exception
    {
        ...
        {
            case MaCnDocDetailAction.DOCLIB_DETAIL_INIT:
                // 페이지 조회
                this.findDetail(request, maDocLibDetailForm);
                returnActionForward = mapping.getInputForward();
                break;
            case MaCnDocDetailAction.DOCLIB_DETAIL_INPUT:
            	insertDetail(maDocLibDetailForm, request);
                returnActionForward = mapping.findForward("ajaxXmlVal");
            	break;
            case MaCnDocDetailAction.DOCLIB_DETAIL_UPDATE:
            	updateDetail(maDocLibDetailForm, request);
                returnActionForward = mapping.findForward("ajaxXmlVal");
            	break;
            case MaCnDocDetailAction.DOCLIB_FILE_SAVE:
                fileUpload(maDocLibDetailForm, request, response);
                returnActionForward = mapping.findForward("jsonPage");
                break;
            case MaCnDocDetailAction.DOCLIB_FILE_DELETE:
                fileDelete(maDocLibDetailForm, request, response);
                returnActionForward = mapping.findForward("ajaxXmlVal");
                break;
            case MaCnDocDetailAction.DOCLIB_FILE_SAVE_TEST:
