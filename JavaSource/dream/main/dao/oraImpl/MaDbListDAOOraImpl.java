 ...
 */
public class MaDbListDAOOraImpl extends BaseJdbcDaoSupportOra implements MaDbListDAO
{
    /**
     * grid find
     * @author  
    ...
    { 
        ...
        if(resultMap.containsKey("NWW"))
        {
            ...
            query.append("  AND x.is_deleted = 'N'                         ");
            query.append("  AND x.wo_type NOT IN ( 'PMI' )                  ");
            query.append("  AND wkor_date >= TO_CHAR(TRUNC(SYSDATE, 'IW')+0 ,'yyyymmdd')        ");
            query.append("  AND wkor_date <= TO_CHAR(SYSDATE,'yyyymmdd')         				");
            if( (!"".equals(user.getEqLocId())&&!"null".equals(user.getEqLocId())) 
                    || (!"".equals(user.getEqLocDesc())&&!"null".equals(user.getEqLocDesc())) )
            {
                query.append("AND x.wkor_id IN (SELECT a.wkor_id                        ");
        ...
        if(resultMap.containsKey("NPMCW"))
        {
            ...
            query.append("  AND x.is_deleted = 'N'                         ");
        	query.append("  AND  x.wo_type = 'PMC'							");
            query.append("  AND x.wkor_date >= TO_CHAR(TRUNC(SYSDATE, 'IW')+0 ,'yyyymmdd')        ");
            query.append("  AND x.wkor_date <= TO_CHAR(SYSDATE,'yyyymmdd')         				");
            if( (!"".equals(user.getEqLocId())&&!"null".equals(user.getEqLocId())) 
                    || (!"".equals(user.getEqLocDesc())&&!"null".equals(user.getEqLocDesc())) )
            {
                query.append("AND x.wkor_id IN (SELECT a.wkor_id                        ");
        ...
        if(resultMap.containsKey("SPRD"))
        {
            ...
            query.append("WHERE REPLACE(SUBSTR(budat,0, 10),'-','') = TO_CHAR(SYSDATE ,'yyyyMMdd')       ");
        ...
        if(resultMap.containsKey("SPRW"))
        {
            ...
            query.append("        COUNT(1) count     						 ");
            query.append("FROM TXPTERPRECLIST      					    	 ");
            query.append("WHERE REPLACE(SUBSTR(budat,0, 10),'-','') >= TO_CHAR(TRUNC(SYSDATE, 'IW'),'yyyyMMdd')          ");
            query.append("    AND REPLACE(SUBSTR(budat,0, 10),'-','') <= TO_CHAR(TRUNC(SYSDATE, 'IW')+6 ,'yyyyMMdd')    ");
