 ...
 */
public class BoardPopupListDAOSqlImpl extends BaseJdbcDaoSupportSql implements BoardPopupListDAO
{
    /**
     * grid find
     * @author  pochul2423
    ...
    {        
        QuerySqlBuffer QuerySqlBuffer = new QuerySqlBuffer();

        // CommonDTO의 boardNo가 있는경우는 상세에서 수정 되어서 List의 한 Row만을 재조회하는 경우이다.
        if (!"".equals(BoardPopupCommonDTO.getBoardNo()))
        {
            QuerySqlBuffer.getAndQuery("x.board_no", BoardPopupCommonDTO.getBoardNo()); 
            return QuerySqlBuffer.toString();
        }

        QuerySqlBuffer.getAndQuery("x.bd_view_no", BoardPopupCommonDTO.getFilterBoardNo()); //게시번호
        QuerySqlBuffer.getAndQuery("x.description", BoardPopupCommonDTO.getDescription()); //필터의 관리번호
        QuerySqlBuffer.getAndQuery("x.enter_by",    BoardPopupCommonDTO.getEnterBy());     //구분
        QuerySqlBuffer.getAndDateQuery("x.enter_date", BoardPopupCommonDTO.getEnterDateFrom(), BoardPopupCommonDTO.getEnterDateTo());
