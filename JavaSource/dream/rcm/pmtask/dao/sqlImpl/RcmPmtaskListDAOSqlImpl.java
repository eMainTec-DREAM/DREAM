 ...
 */
public class RcmPmtaskListDAOSqlImpl extends BaseJdbcDaoSupportSql implements RcmPmtaskListDAO
{
    /**
     * grid find
     * @author  kim21017
    ...
    public List findList(RcmPmtaskCommonDTO rcmPmtaskCommonDTO, User user)
    {
        ...
        query.append("       dbo.SFACODE_TO_DESC(h.rcm_task_type,'RCM_TASK_TYPE','SYS','',?) rcmTaskTypeDesc,    			");
        query.append("       h.rcm_task_type rcmTaskType,    	");
        query.append("       CONCAT(cycle,' ',dbo.SFACODE_TO_DESC(h.period_type,'PERIOD_TYPE','SYS','',?)) cycleDesc,    	");
        ...
        return getJdbcTemplate().queryForList(query.toString(rcmPmtaskCommonDTO.getIsLoadMaxCount(), rcmPmtaskCommonDTO.getFirstRow()), getObject(objects));
    }
