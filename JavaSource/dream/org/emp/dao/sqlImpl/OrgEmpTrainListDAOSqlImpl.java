 ...
 */
public class OrgEmpTrainListDAOSqlImpl extends BaseJdbcDaoSupportSql implements OrgEmpTrainListDAO
{
    /**
     * grid find
     * @author  kim21017
    ...
    public List findItemList(MaEmpCommonDTO maEmpCommonDTO, OrgEmpTrainListDTO orgEmpTrainListDTO, User user)
    {
        ...
        query.append("      , CASE WHEN x.train_fdate='' THEN '' ELSE SUBSTRING(x.train_fdate, 1, 4)+'-'+SUBSTRING(x.train_fdate, 5, 2)+'-'+SUBSTRING(x.train_fdate, 7, 2) END+' ~ '+    ");
        query.append("        CASE WHEN x.train_tdate='' THEN '' ELSE SUBSTRING(x.train_tdate, 1, 4)+'-'+SUBSTRING(x.train_tdate, 5, 2)+'-'+SUBSTRING(x.train_tdate, 7, 2) END trainDate ");
        ...
        query.getOrderByQuery("x.emptrainlist_id","x.train_fdate", maEmpCommonDTO.getOrderBy(), maEmpCommonDTO.getDirection());
        
        return getJdbcTemplate().queryForList(query.toString(orgEmpTrainListDTO.getIsLoadMaxCount(), orgEmpTrainListDTO.getFirstRow()));
    }
