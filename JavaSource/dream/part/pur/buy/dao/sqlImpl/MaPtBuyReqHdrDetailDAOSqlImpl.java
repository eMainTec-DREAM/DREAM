 ...
 */
public class MaPtBuyReqHdrDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaPtBuyReqHdrDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public MaPtBuyReqHdrDetailDTO findDetail(MaPtBuyReqHdrCommonDTO maPtBuyReqHdrCommonDTO,User user)
    {
        ...
        query.append("		 ,dbo.SFACODE_TO_DESC(x.ptprlist_status,'PTPRLIST_STATUS','SYS','','"+user.getLangId()+"')	AS ptPrListStatusDesc	");
    ...
    public String checkDetail(MaPtBuyReqHdrDetailDTO maPtBuyReqHdrDetailDTO)
    {
    	...
    	query.append("SELECT count(ISNULL(rec_qty,0))			");
    ...
    public List findReportPtDetail(MaPtBuyReqHdrDetailDTO maPtBuyReqHdrDetailDTO) {
        ...
        query.append("		(SELECT SUM((ISNULL(a.unit_price,0)*a.rec_qty))																		");
        query.append("			FROM TAPTPRITEM a																								");
        query.append("			WHERE a.comp_no = x.comp_no																						");
        query.append("			AND a.ptprlist_id = x.ptprlist_id) totalPrice,																	");
        query.append("		CONVERT(VARCHAR(7), CONVERT(DATE, x.req_date), 111) yyyymm,															");
        query.append("		CONVERT(VARCHAR, CONVERT(DATE, x.req_date), 23) yyyymmdd,															");
        query.append("		RIGHT(CONVERT(VARCHAR, getdate(), 103), 8) mmyyyy,																	");
    ...
    public List findReportPartList(MaPtBuyReqHdrDetailDTO maPtBuyReqHdrDetailDTO) {
		...
		query.append("SELECT											");
		query.append("		ROW_NUMBER() OVER (ORDER BY ptpritem_id) seqNum,	");
		query.append("		(SELECT part_no 							");
		query.append("		   FROM TAPARTS								");
		query.append("		WHERE comp_no = x.comp_no					");
		query.append("		AND part_id = x.part_id) partNum,			");
		
		query.append("		CASE x.part_id WHEN '' THEN x.description ELSE	");
		query.append("		(SELECT description+', '+ISNULL(pt_size,'')	");
		query.append("		   FROM TAPARTS								");
		query.append("		WHERE comp_no = x.comp_no					");
		query.append("		AND part_id = x.part_id) END ptNameSize,	");
