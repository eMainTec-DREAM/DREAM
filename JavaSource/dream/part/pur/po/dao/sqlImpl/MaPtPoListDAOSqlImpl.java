 ...
 */
public class MaPtPoListDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaPtPoListDAO
{
    /**
     * grid find
     * @author  kim21017
    ...
    public List findList(MaPtPoCommonDTO maPtPoCommonDTO, User user)
    {
        ...
        query.append("       dbo.SFACODE_TO_DESC(x.polist_status, 'PO_STATUS', 'SYS', x.comp_no,'"+user.getLangId()+"') poListStatus, ");
        ...
        query.getOrderByQuery("x.polist_id","x.po_date DESC", maPtPoCommonDTO.getOrderBy(), maPtPoCommonDTO.getDirection());
    ...
    public int insertRec(MaPtPoCommonDTO maPtPoCommonDTO, String poListId, User user)
    {
    	...
    	query = new QuerySqlBuffer();
		query.append("SELECT NEXT VALUE FOR SQAFCRECLIST_ID		");
        
		String recId = QuerySqlBuffer.listToString(this.getJdbcTemplate().queryForList(query.toString()));
		
		query = new QuerySqlBuffer();
		query.append("SELECT NEXT VALUE FOR SQAPTRECHIST_ID		");
		
		String histId = QuerySqlBuffer.listToString(this.getJdbcTemplate().queryForList(query.toString()));
