 ...
 */
public class MaPtIssDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaPtIssDetailDAO
{
	public MaPtIssDetailDTO findDetail(MaPtIssCommonDTO maPtIssCommonDTO,User user)
    {
        ...
        query.append("     , dbo.SFACODE_TO_DESC(x.ptiss_status, 'PTISS_STATUS', 'SYS', '', '"+user.getLangId()+"')	AS PTISSSTATUSDESC	");
        ...
        query.append("     , dbo.SFACODE_TO_DESC(x.ptiss_type, 'PTISS_TYPE', 'SYS', '', '"+user.getLangId()+"')     AS PTISSTYPEDESC    ");
        ...
        MaPtIssDetailDTO maPtIssDetailDTO = (MaPtIssDetailDTO)getJdbcTemplate().query(query.toString(),objects, new MwareExtractor(new MaPtIssDetailDTO()));
	...
	public boolean checkWorkOrderStatus(MaPtIssDetailDTO maPtIssDetailDTO, User user)
    {
    	...
    	return QuerySqlBuffer.haveData(getJdbcTemplate().queryForList(query.toString(),objects) );
    }
	...
	public int updateWoPart(MaPtIssDetailDTO maPtIssDetailDTO, User user)
    {
        ...
        query.append("                  FROM TAPARTS                                     ");
        query.append("                  WHERE comp_no = ?                                ");
        query.append("                    AND part_id = ?)*isnull(use_qty,0))            ");
	...
	public int updatePtIssList(MaPtIssDetailDTO maPtIssDetailDTO, User user)
    {
        ...
        query.append("                    FROM TAPARTS                                     						");
        query.append("                   WHERE comp_no = ?                                						");
        query.append("                     AND part_id = ?)*isnull(issue_qty,0))               					");
	...
	public void execSP_PT_OUTSTOCK(String ptisshistId, User user)
    {
        QuerySqlBuffer query = new QuerySqlBuffer();
        query.append("EXEC                                                           ");
        query.append("dbo.SP_PT_OUTSTOCK                                              ");
    ...
    public String getUseQty(MaPtIssDetailDTO maPtIssDetailDTO, User user)
    {
        ...
        query.append("SELECT ISNULL(SUM(use_qty),0)");
    ...
    public int updateEqPart(MaPtIssDetailDTO maPtIssDetailDTO, User user)
    {
        ...
        query.append("            ,ISNULL(x.eqasmb_id, z.eqasmb_id) eqAsmbId                ");
        ...
        query.append("  AND ISNULL(a.eqasmb_id,0) = ISNULL(b.eqAsmbId,0)                    ");
        query.append("  AND a.part_id = b.partId)                                           ");
        query.append("WHEN MATCHED THEN                                                     ");
        query.append("  UPDATE SET a.issue_time = ISNULL(a.issue_time,0) + 1                ");
        query.append("             ,a.issue_last_date = ?                                   ");
        query.append("             ,a.use_qty = ISNULL(a.use_qty,0) + ?                     ");
        query.append("WHEN NOT MATCHED THEN                                                 ");
        query.append("INSERT (comp_no, equip_id, eqasmb_id, part_id, consist_qty, use_qty, issue_time, issue_first_date, issue_last_date, is_eqtype_part, is_use)           ");
        query.append("VALUES (b.compNo, b.equipId,b.eqAsmbId, b.partId, 1, ?, 1, ?, ?, 'N', 'Y' )           ");
        query.append(";");
