 ...
 */
public class LovPartsListDAOSqlImpl extends BaseJdbcDaoSupportSql implements LovPartsListDAO
{
    /**
     * 자재 검색
     * @author  kim21017
    ...
    public List findPartsList(LovPartsListDTO lovPartsListDTO, User loginUser)
    {
        ...
        query.append("       REPLACE(CONVERT(VARCHAR(50), CAST(last_price AS MONEY), 1) , '.00', '')     lastPrice,		");
        ...
        query.getOrderByQuery("x.part_id","x.part_no", lovPartsListDTO.getOrderBy(), lovPartsListDTO.getDirection());
			...
			Map<String, String> conditionMap)
    {
        ...
        query.append("       ,dbo.SFACODE_TO_DESC(x.uom, 'UOM', 'USR', x.comp_no,'"+loginUser.getLangId()+"') AS uom_desc ");
        query.append("       ,plf_type 		AS plf_type									");
        query.append("       ,dbo.SFACODE_TO_DESC(plf_type,'PLF_TYPE','SYS','','"+loginUser.getLangId()+"') AS plf_type_desc	");
        ...
        query.append("       ,dbo.SFACODE_TO_DESC(currency,'CURRENCY','SYS','','"+ loginUser.getLangId() +"')	AS currencyDesc	");
        query.append("FROM TAPARTS	x													");
        query.append("WHERE 1=1															");

        query.append(this.getWhere(lovPartsListDTO, loginUser, columnMap, conditionMap));

        query.getOrderByQuery("x.part_id","x.part_id", lovPartsListDTO.getOrderBy(), lovPartsListDTO.getDirection());
    ...
    {        
        ...
        if(!"".equals(lovPartsListDTO.getFilterEqAsmbId()) || !"".equals(lovPartsListDTO.getFilterEqAsmbDesc()) )
        {
        	...
        	if(!"".equals(lovPartsListDTO.getFilterEqAsmbId()))
        	{
                ...
                query.append("                 FROM dbo.SFAEQASMB_CALL('"+user.getCompNo()+"','"+lovPartsListDTO.getFilterEqAsmbId()+"','"+lovPartsListDTO.getFilterEqAsmbDesc()+"') c	");
        	...
        	{
        		query.append("AND exists (   SELECT c.eqasmb_id                             ");
                query.append("                 FROM dbo.SFAEQASMB_CALL('"+user.getCompNo()+"','','"+lovPartsListDTO.getFilterEqAsmbDesc()+"') c	");
                query.append("                WHERE c.comp_no = b.comp_no            		");
                query.append("            )                                           		");
        	}
        	query.append("           )														");
