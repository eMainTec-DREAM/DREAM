 ...
 */
public class MaPtStckDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaPtStckDetailDAO
{
    /**
     * detail find
     * @author ssong
    ...
    public MaPtStckDetailDTO findDetail(MaPtStckCommonDTO maPtStckCommonDTO)
    {
        ...
        query.append("       SUM(CASE x.part_grade WHEN 'A' THEN x.stock_qty ELSE 0 END) aStockQty,  "); //a현재고
        query.append("       SUM(CASE x.part_grade WHEN 'B' THEN x.stock_qty ELSE 0 END) bStockQty,  "); //b현재고
        query.append("       MIN(y.safty_qty)                  minSaftyQty,     "); //안전재고 min
        query.append("       MIN(y.max_safty_qty)              maxSaftyQty,     "); //안전재고 max
        query.append("       MAX(CASE x.part_grade WHEN 'A' THEN x.bin_no ELSE '' END) abinNo,  "); //a현재고
        query.append("       MAX(CASE x.part_grade WHEN 'B' THEN x.bin_no ELSE '' END) bbinNo   "); //b현재고
        query.append("FROM   TAPTSTOCK x LEFT OUTER JOIN                        ");
    ...
    public int insertPtStock(MaPtStckDetailDTO maPtStckDetailDTO,String stockQty,String partGrade)
    {
    	...
    	query.append(")	VALUES (                                                ");
    	query.append("  ?,         ?,          ?,          ?,                   ");
    	query.append("  ?,         CASE ? WHEN 'A' THEN ? WHEN 'B' THEN ? ELSE '' END	");
    	query.append(")                                                         ");
            ...
            MaPtStckDetailDTO maPtStckDetailDTO, User loginUser)
    {
        ...
        query.append("       x.last_price,   ?*x.last_price						");
    ...
    public List findReportPtDetail(MaPtStckDetailDTO maPtStckDetailDTO,User user) {
        ...
        query.append("		CONVERT(VARCHAR(7), getdate(), 111) yyyymm,																			");
        query.append("		CONVERT(VARCHAR, getdate(), 23) yyyymmdd,																			");
    ...
    public List findReportPartList(MaPtStckDetailDTO maPtStckDetailDTO,User user) {
		...
		query.append("SELECT											");
		query.append("		ROW_NUMBER() OVER (ORDER BY x.part_id) seqNum,");
