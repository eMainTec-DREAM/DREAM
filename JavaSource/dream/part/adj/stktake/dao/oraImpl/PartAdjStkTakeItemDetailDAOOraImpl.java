 ...
 */
public class PartAdjStkTakeItemDetailDAOOraImpl extends BaseJdbcDaoSupportOra implements PartAdjStkTakeItemDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public PartAdjStkTakeItemDetailDTO findDetail(PartAdjStkTakeListDTO partAdjStkTakeListDTO, PartAdjStkTakeCommonDTO partAdjStkTakeCommonDTO, User user)
    {
        ...
        query.append("      , x.stock_qty			                                                                          AS sstockQty		 ");
        query.append("      , x.real_qty			                                                                          AS realQty		 ");
        query.append("      , x.gap_qty			                                                                              AS gapQty			 ");
	...
	public int insertGap(PartAdjStkTakeItemDetailDTO partAdjStkTakeItemDetailDTO,PartAdjStkTakeCommonDTO partAdjStkTakeCommonDTO, User user) {
    	...
    	query.append("UPDATE TAPTSTKTAKEITEM x SET                		");
    	query.append("    x.stock_qty                    =        NVL((SELECT a.stock_qty                                     		");
    	query.append("                                                            FROM TAPTSTOCK a                                           		");
    	query.append("                                                            WHERE a.part_id = x.part_id                                		");
    	query.append("                                                            AND a.part_grade = x.part_grade AND a.comp_no =x.comp_no AND a.wcode_id = (SELECT z.wcode_id FROM TAPTSTKTAKELIST z WHERE x.ptstktakelist_id=z.ptstktakelist_id)),0)   ,    		");
    	query.append("    x.gap_qty                = x.real_qty -  NVL((SELECT a.stock_qty                                     		");
    	query.append("                                                            FROM TAPTSTOCK a                                           		");
    	query.append("                                                            WHERE a.part_id = x.part_id                                		");
    	query.append("                                                            AND a.part_grade = x.part_grade AND a.comp_no =x.comp_no AND a.wcode_id = (SELECT z.wcode_id FROM TAPTSTKTAKELIST z WHERE x.ptstktakelist_id=z.ptstktakelist_id)),0)       		");
    	query.append("WHERE x.ptstktakeitem_id         = ?     		");
	...
	public int insertDefaultItem(PartAdjStkTakeCommonDTO partAdjStkTakeCommonDTO, User user)
	{
		...
		query.append(" )                                	            ");
		query.append("SELECT                      						");
		query.append("      x.comp_no			,sqaptstktakeitem_id.NEXTVAL	");
