 ...
 */
public class WorkServicePriceDAOOraImpl extends BaseJdbcDaoSupportOra implements WorkServicePriceDAO
{
    /**
     * grid find
     * @author  youngjoo38
	...
	@Override
	public String getColums(WorkServicePriceDTO workServicePriceDTO, User loginUser) {
		QueryBuffer query = new QueryBuffer();
		...
		query.append("    , TO_CHAR(TO_DATE(x.start_month,'yyyymm'),'yyyy-mm') 		");
		query.append("        || ' ~ ' || 											");
		query.append("           TO_CHAR(TO_DATE(x.end_month,'yyyymm'),'yyyy-mm')     periodMonth		");
	...
	public String getOrderBy(WorkServicePriceDTO workServicePriceDTO, User user) {
		...
		query.getOrderByQuery("NVL(x.start_month,x.serviceprice_id)", workServicePriceDTO.getOrderBy(), workServicePriceDTO.getDirection());		
