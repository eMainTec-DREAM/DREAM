 ...
 */
public class MaStdWoTypeDetailDAOSqlImpl extends BaseJdbcDaoSupportOra implements MaStdWoTypeDetailDAO
{
    /**
     * detail find
     * @author 
    ...
    public MaStdWoTypeDetailDTO findDetail(MaStdPointCommonDTO maStdPointCommonDTO, MaStdWoTypeListDTO maStdWoTypeListDTO, User loginUser)
    {
        ...
        query.append("		, (SELECT dbo.SFACODE_TO_DESC(x.wo_type,'WO_TYPE','SYS','','"+loginUser.getLangId()+"') ) woTypeDesc		");
        query.append("		, x.pm_type 				pmType		");
        query.append("		, (SELECT dbo.SFACODE_TO_DESC(x.pm_type, x.wo_type+'_TYPE','SYS','','"+loginUser.getLangId()+"') ) pmTypeDesc		");
