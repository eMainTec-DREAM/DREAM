 ...
 */
public class MaStdPointHdrListDAOOraImpl extends BaseJdbcDaoSupportOra implements MaStdPointHdrListDAO
{
    /**
     * grid find
     * @author  
    ...
    { 
        ...
        query.append("      ,(SELECT SFACODE_TO_DESC(x.revision_status,'REVISION_STATUS','SYS','','"+loginUser.getLangId()+"') FROM DUAL)    revisionStatus    ");
        ...
        query.append("      ,(SELECT SFACODE_TO_DESC(x.is_active, 'IS_USE', 'SYS', x.comp_no,'"+loginUser.getLangId()+"') FROM DUAL) isActiveDesc		");
        query.append("FROM   TASTWRKLST x														");
        query.append("WHERE  1=1																"); 
        query.append(this.getWhere(maStdPointCommonDTO,loginUser));
        query.getOrderByQuery("x.stwrk_no", maStdPointCommonDTO.getOrderBy(), maStdPointCommonDTO.getDirection());
	...
	public int insertCopyDetail(String newId, String oldId,String revisionHistId,String revisionStatus, User user, String isCopy)
    {
    	...
    	query.append("	stwrk_status, eqctg_id,	dept_id, TO_CHAR(SYSDATE, 'YYYYMMDD'),	");
    	query.append(" 	?,  	?, 		remark, 	?,									");
    	query.append("	?,		? 														");
	...
	public int insertCopyPoint(String newId, String oldId, User user, String isCopy)
    {
    	...
    	query.append(")																		");
    	query.append("SELECT																");
    	query.append("	 comp_no,		?,	SQASTWRK_POINT_ID.NEXTVAL,			");
    	if("Y".equals(isCopy))
    	{
    		// 복사
	...
	public int insertCopyWoType(String newId, String oldId, User user)
    {
    	...
    	query.append(")															");
    	query.append("SELECT													");
    	query.append("	comp_no,	?,				SQASTWRKWORK_ID.NEXTVAL,	");
    	query.append(" 	wo_type,	description, 	remark						");
	...
	public int insertCopyWork(String newId, String oldId, User user)
    {
    	...
    	query.append(")															");
    	query.append("SELECT													");
    	query.append("	comp_no,		?,				SQASTWRKWORKPRC_ID.NEXTVAL,	");
    	query.append(" 	stwrkwork_id,	step_num, 		eqasmb_desc,			");
    	query.append(" 	description,	remark									");
