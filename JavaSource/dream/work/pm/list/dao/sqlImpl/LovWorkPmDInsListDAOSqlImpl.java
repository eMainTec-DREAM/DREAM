 ...
 */
public class LovWorkPmDInsListDAOSqlImpl extends BaseJdbcDaoSupportSql implements LovWorkPmDInsListDAO
{
    /**
     * 검색
     * @author  euna0207
	...
	public List findEqCtgPartAcList(LovWorkPmDInsListDTO lovWorkPmDInsListDTO, User user, Map<String, String> conditionMap) throws Exception
    {
		...
		query.append(" , x.eqctgpmpoint_id                                                                                     		AS eqCtgPmPointId   ");//LOV KEY
		query.append(" , (SELECT eqctg_id FROM TAEQCTG WHERE comp_no = x.comp_no AND eqctg_id = x.eqctg_id)                    		AS eqCtgId          ");//설비 KEY
		query.append(" , x.cycle                                                                                                 	AS cycle            ");
		query.append(" , (SELECT top 1 description FROM TAEQASMB WHERE comp_no = x.comp_no AND eq_ctg_asmb_id = x.eq_ctg_asmb_id)         AS eqasmbDesc       ");
	...
	private String getWhere(LovWorkPmDInsListDTO lovWorkPmDInsListDTO, User user, Map<String, String> conditionMap) throws Exception
	{
		...
		if(!"".equals(conditionMap.get("x.eqctg_id")) && conditionMap.get("x.eqctg_id") != null )
		{
