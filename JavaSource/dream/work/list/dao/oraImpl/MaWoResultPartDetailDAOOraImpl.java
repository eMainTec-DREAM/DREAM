 ...
 */
public class MaWoResultPartDetailDAOOraImpl extends BaseJdbcDaoSupportOra implements MaWoResultPartDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public int insertDetail(MaWoResultPartDetailDTO maWoResultPartDetailDTO, MaWoResultMstrCommonDTO maWoResultMstrCommonDTO)
    {
    	...
    	query.append("	 ?,				?,							");
    	query.append("	 ?,				?,							");
    	query.append("	CASE WHEN (SELECT NVL(SUM(unit_price),0)	");
    	query.append("				FROM TAPTSTOCK					");
    	...
    	query.append("				) = 0							");
    	query.append("		THEN (SELECT NVL(SUM(last_price),0)		");
    	query.append("				FROM TAPARTS 					");
    	query.append("				WHERE comp_no = ?				");
    	query.append("				AND   part_id = ?				");
    	query.append("			)									");
    	query.append("		ELSE (SELECT NVL(SUM(unit_price),0)		");
    	query.append("				FROM TAPTSTOCK					");
    	query.append("				WHERE comp_no 	= ?				");
    	query.append("				AND   wcode_id 	= ?				");
    	query.append("				AND   part_id 	= ?				");
    ...
    public int insertIssPartDetail(MaWoResultPartDetailDTO maWoResultPartDetailDTO, MaWoResultMstrCommonDTO maWoResultMstrCommonDTO)
    {
        ...
        query.append("       ,?                                                 ");
        query.append("       ,?                                                 ");
        query.append("       ,nvl(to_wcode_id,wcode_id)  as wcode_id            ");
    ...
    public List getIssQty(MaWoResultPartDetailDTO maWoResultPartDetailDTO, User loginUser)
    {
        ...
        query.append("  ,NVL(sum(b.use_qty),0) 	AS useQty               ");
