 ...
 */
public class WoPlanDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements WoPlanDetailDAO
{
    /**
     * detail find
     * @author youngjoo38
    ...
    public WoPlanDetailDTO findDetail(WoPlanCommonDTO woPlanCommonDTO, User user)
    {
        ...
        query.append("     , dbo.SFACODE_TO_DESC(x.woplan_status, 'WOPLAN_STATUS', 'SYS', '', '"+ user.getLangId() +"')			AS WOPLANSTATUSDESC		");
        ...
        query.append("     , dbo.SFACODE_TO_DESC(x.self_vendor_type, 'SELF_VENDOR_TYPE', 'SYS', '', '"+ user.getLangId() +"')	AS SELFVENDORTYPEDESC	");
        ...
        query.append("     , dbo.SFAIDTODESC(x.vendor_id, '', 'VENDOR', x.comp_no)   											AS VENDORDESC  			");
        ...
        WoPlanDetailDTO woPlanDetailDTO = (WoPlanDetailDTO)getJdbcTemplate().query(query.toString(), getObject(objects), new MwareExtractor(new WoPlanDetailDTO()));
    ...
    public int completeDetail(WoPlanDetailDTO woPlanDetailDTO, User user)
    {
        ...
        query.append("    , complete_date       = CONVERT(VARCHAR, GETDATE(), 112)	");
        query.append(" WHERE wkor_id         	= ?     							");
        query.append("   AND comp_no         	= ?     							");
    ...
    public int insertWoPlanCraft(WoPlanDetailDTO woPlanDetailDTO, User user)
    {
        ...
        craftCnt = Integer.parseInt(QuerySqlBuffer.listToString(getJdbcTemplate().queryForList(query.toString(),cntObjects)));
        ...
        if(craftCnt>=1)
        {
            ...
            {
                //System.out.println("작업자 i : " + i);
                // 시퀀스 가져오기
                query.setClear();
                query.append("SELECT NEXT VALUE FOR SQAWOCRAFT_ID  ");
                
                woCraftId = QuerySqlBuffer.listToString(getJdbcTemplate().queryForList(query.toString()));
                ...
                query.append("SELECT a.comp_no, ?, a.wkor_id, a.emp_id, a.start_date, a.start_time, a.end_date, a.end_time, a.work_time, a.remark        ");
                query.append("FROM (                                ");
                query.append("    SELECT ROW_NUMBER() OVER( ORDER BY x.wkor_id) AS rno, x.comp_no, x.wkor_id, x.emp_id, x.start_date, x.start_time, x.end_date, x.end_time, x.work_time, x.remark        ");
                query.append("    FROM TAWOPLANCRAFT x              ");
                query.append("    WHERE x.wkor_id = ?               ");
//                query.append("    ORDER BY x.wkor_id                ");
                query.append("     ) a                               ");
                query.append("WHERE a.rno = '"+i+"'                   ");
    ...
    public int insertWoPlanParts(WoPlanDetailDTO woPlanDetailDTO, User user)
    {
        ...
        partCnt = Integer.parseInt(QuerySqlBuffer.listToString(getJdbcTemplate().queryForList(query.toString(),cntObjects)));
        //System.out.println("partCnt : " + partCnt);
        // 등록된 부품이 0개가 아니면
        if(partCnt>=1)
        {
            // 등록된 부품갯수만큼 INSERT한다.
            ...
            {
                //System.out.println("부품 i : " + i);
                // 시퀀스 가져오기
                query.setClear();
                query.append("SELECT NEXT VALUE FOR SQAWOPART_ID   ");
                
                woPartId = QuerySqlBuffer.listToString(getJdbcTemplate().queryForList(query.toString()));
                
                // TAWOPLANPARTS에서 TAWOPARTS로 데이터 INSERT (갯수만큼)
                query.setClear();
                query.append("INSERT INTO TAWOPARTS (comp_no, wopart_id, wkor_id, wcode_id, part_id, part_grade, use_qty, REMARK)       ");
                query.append("SELECT a.comp_no, ?, a.wkor_id, a.wcode_id, a.part_id, a.part_grade, a.use_qty, a.remark                ");
                query.append("FROM (                                        ");
                query.append("    SELECT ROW_NUMBER() OVER( ORDER BY x.wkor_id) AS rno, x.comp_no, x.wkor_id, x.wcode_id, x.part_id, x.part_grade, x.use_qty, x.remark       ");
                query.append("    FROM TAWOPLANPARTS x                      ");
                query.append("    WHERE x.wkor_id = ?                       ");
