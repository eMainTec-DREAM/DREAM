 ...
 */
public class WorkCalPmInsApprRsltListDAOOraImpl extends BaseJdbcDaoSupportOra implements WorkCalPmInsApprRsltListDAO
{
    public List findList(WorkCalPmInsApprCommonDTO workCalPmInsApprCommonDTO,WorkCalPmInsApprDetailDTO workCalPmInsApprDetailDTO, User user)
    {
        ...
        query.append("   , SFACODE_TO_DESC(b.pmsched_status  , 'PMSCHED_STATUS', 'SYS', b.comp_no, '"+user.getLangId()+"')          AS pmSchedStatus                                                                ");
        query.append("   , CASE WHEN e.start_time IS NOT NULL                                                                                                                                                       ");
        query.append("          THEN SUBSTR(e.start_date,0,4) || '-' || SUBSTR(e.start_date,5,2) || '-' || SUBSTR(e.start_date,7,2) ||' '|| SUBSTR(e.start_time,0,2) ||':' || SUBSTR(e.start_time,3,2)              ");
        query.append("          ELSE (CASE WHEN e.start_date IS NOT NULL THEN SUBSTR(e.start_date,0,4) || '-' || SUBSTR(e.start_date,5,2) || '-' || SUBSTR(e.start_date,7,2) ELSE '' END)                           ");
        query.append("          END                                                                                                 AS inspectFromDate                                                              ");
        query.append("   , CASE WHEN e.end_time IS NOT NULL                                                                                                                                                         ");
        query.append("          THEN SUBSTR(e.end_date,0,4) || '-' || SUBSTR(e.end_date,5,2) || '-' || SUBSTR(e.end_date,7,2) ||' '|| SUBSTR(e.end_time,0,2) ||':' || SUBSTR(e.end_time,3,2)                        ");
        query.append("          ELSE (CASE WHEN e.end_date IS NOT NULL THEN SUBSTR(e.end_date,0,4) || '-' || SUBSTR(e.end_date,5,2) || '-' || SUBSTR(e.end_date,7,2) ELSE '' END)                                   ");
        ...
        query.append("   , SFACODE_TO_DESC(d.check_type,'CHECK_TYPE','SYS','','"+user.getLangId()+"')                               AS checkTypeDesc                                                                ");
        query.append("   , SFACODE_TO_DESC(TRIM(f.pm_point_rslt_status),'PM_POINT_RSLT_STATUS','SYS','','"+user.getLangId()+"')     AS pmPointRsltStatusDesc                                                        ");
        ...
        query.getOrderByQuery("f.actual_date , (SELECT full_desc FROM TAEQLOC WHERE comp_no = c.comp_no AND eqloc_id = (SELECT eqloc_id FROM TAEQUIPMENT WHERE comp_no = c.comp_no AND equip_id = c.equip_id)), c.equip_id,d.step_num"
