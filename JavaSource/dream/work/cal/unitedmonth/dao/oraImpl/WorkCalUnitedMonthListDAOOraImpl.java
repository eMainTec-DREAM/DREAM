package dream.work.cal.unitedmonth.dao.oraImpl;
 ...
 */
public class WorkCalUnitedMonthListDAOOraImpl extends BaseJdbcDaoSupportOra implements WorkCalUnitedMonthListDAO
{
    /**
     * grid find
     * @author  youngjoo38
    ...
    public List findWorkSchedList(WorkCalUnitedMonthCommonDTO workCalUnitedMonthCommonDTO, MaWoResultMstrCommonDTO maWoResultMstrCommonDTO, User loginUser)
    {
        ...
        query.append("            AND eqloc_id = b.eqloc_id )                    	    eqLocDesc       ");
        query.append("		,DECODE(b.eqctg_type,'MD','('||b.old_eq_no||')'||b.description,b.description) AS equipDesc		");
        ...
        query.getOrderByQuery("x.wkor_date, x.description", workCalUnitedMonthCommonDTO.getOrderBy(), workCalUnitedMonthCommonDTO.getDirection());
    ...
    public List findPmiSchedList(WorkCalUnitedMonthCommonDTO workCalUnitedMonthCommonDTO, MaWoResultMstrCommonDTO maWoResultMstrCommonDTO, User user)
    {
        ...
        query.append("		,(SELECT SFACODE_TO_DESC(x.shift_type,'SHIFT_TYPE','SYS','','"+user.getLangId()+"') FROM DUAL)			AS SHIFTTYPE		");
        query.append("		,(SELECT description													                                ");
        query.append("		    FROM TADEPT															                                ");
        query.append("		   WHERE comp_no = x.comp_no											                                ");
        query.append("			 AND dept_id = x.dept_id) 								AS DEPTDESC	                                ");
        query.append("		,(SELECT SFACODE_TO_DESC(x.pm_type,'PMI_TYPE','SYS','','"+user.getLangId()+"') FROM DUAL) 				AS PMTYPE			");
        query.append("		,x.pmsched_status											AS PMSCHEDSTATUS	                        ");
        query.append("		,(SELECT SFACODE_TO_DESC(x.pmsched_status,'PMSCHED_STATUS','SYS','','"+user.getLangId()+"') FROM DUAL)	AS PMISCHEDSTATUS	");
        ...
        query.append("WHERE 1=1																		                                ");
    	query.append(this.getInsWhere(workCalUnitedMonthCommonDTO, maWoResultMstrCommonDTO, user));
     ...
     * Filter 조건
     * @author  youngjoo38
     * @version $Id: WorkCalUnitedMonthListDAO.java,v 1.0 2015/12/02 09:14:12 youngjoo38 Exp $
     * @since   1.0
     * 
     * @param day
     * @param status
    ...
    {        
    	...
    	query.getCodeLikeQuery("b.main_mng_id", 
    	        "(SELECT emp_name FROM TAEMP WHERE emp_id = b.main_mng_id AND comp_no = '"+user.getCompNo()+"')"
    	        , workCalUnitedMonthCommonDTO.getMainMngId(), workCalUnitedMonthCommonDTO.getMainMngName());
    	
    	//담당자(부)
    	query.getCodeLikeQuery("b.sub_mng_id", 
    	        "(SELECT emp_name FROM TAEMP WHERE emp_id = b.sub_mng_id AND comp_no = '"+user.getCompNo()+"')"
    ...
    private String getInsWhere(WorkCalUnitedMonthCommonDTO workCalUnitedMonthCommonDTO , MaWoResultMstrCommonDTO maWoResultMstrCommonDTO, User user)
    {        
    	QueryBuffer query = new QueryBuffer();
