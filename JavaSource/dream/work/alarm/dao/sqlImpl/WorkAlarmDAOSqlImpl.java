package dream.work.alarm.dao.sqlImpl;
 ...
 */
public class WorkAlarmDAOSqlImpl extends BaseJdbcDaoSupportSql implements WorkAlarmDAO
{
	...
	public String getColums(WorkAlarmDTO workAlarmDTO, User user) {
		...
		query.append("        , dbo.SFACODE_TO_DESC(x.alarm_channel, 'ALARM_CHANNEL', 'SYS', x.comp_no, '"+ user.getLangId() +"') 	AS ALARMOCCH		");
	...
	public String getOrderBy(WorkAlarmDTO workAlarmDTO, User user) {
		...
		query.getOrderByQuery("x.alarmlist_id", "x.alarmlist_id DESC", workAlarmDTO.getOrderBy(), workAlarmDTO.getDirection());		
	...
	public String getWhere(WorkAlarmDTO workAlarmDTO, User user) {
		...
		// 발생일자
		query.getAndDateQuery("SUBSTRING(x.start_time, 1, 8)", workAlarmDTO.getFilterFromDate(), workAlarmDTO.getFilterToDate());
