package dream.work.let.dao.sqlImpl;
 ...
 */
public class WorkLetListDAOSqlImpl extends BaseJdbcDaoSupportSql implements WorkLetListDAO
{
    /**
     * grid find
     * @author  syyang
    ...
    public List findWoLetList(WorkLetCommonDTO workLetCommonDTO, User user)
    {
        ...
        query.append("		,dbo.SFACODE_TO_DESC(x.wolet_status,'WOLET_STATUS','SYS','','"+user.getLangId()+"')	woLetStatusDesc	");
        query.append("		,x.place 													place		");
        query.append("		,( CONVERT(NVARCHAR, CONVERT(DATETIME, x.start_date, 120) , 23) +' '+ SUBSTRING(x.start_time,1,2)+':'+SUBSTRING(x.start_time,3,4)	");
        query.append("			+ ' ~ '																");
        query.append("		   	+ CONVERT(NVARCHAR, CONVERT(DATETIME, x.end_date, 120) , 23) +' '+ SUBSTRING(x.end_time,1,2)+':'+SUBSTRING(x.end_time,3,4)	");
        ...
        query.getOrderByQuery("x.wolet_id", "x.wolet_no", workLetCommonDTO.getOrderBy(), workLetCommonDTO.getDirection());
    ...
    {        
        ...
        //신청자
        query.getCodeLikeQuery("x.req_by", "(SELECT a.emp_name FROM TAEMP a WHERE a.emp_id = x.emp_id AND a.comp_no=x.comp_no)", 
