 ...
 */
public class WorkRptPmwRateListDAOSqlImpl  extends BaseJdbcDaoSupportSql implements WorkRptPmwRateListDAO
{
    ...
    public List findList(WorkRptPmwRateCommonDTO workRptPmwRateCommonDTO, User user) throws Exception
    {
        ...
        query.append("  ,LEFT(b.wkor_date,6) AS yyyymm     ");
        query.append("  ,sum(CASE WHEN a.PMSCHED_STATUS = 'C' THEN 1 ELSE 0 END) AS c_cnt       ");
        query.append("  ,count(*)  AS t_cnt     ");
        query.append("  ,convert(numeric(5,2),((sum(CASE WHEN a.PMSCHED_STATUS = 'C' THEN 1.0 ELSE 0 END)) / count(*)) *100) AS c_rate      ");
        ...
        query.append(" GROUP BY c.comp_no,c.plant,LEFT(b.wkor_date,6)        ");
        query.getOrderByQuery("c.comp_no", "c.comp_no,c.plant,LEFT(b.wkor_date,6)", workRptPmwRateCommonDTO.getOrderBy(), workRptPmwRateCommonDTO.getDirection());
    ...
    public String findTotalCount(
        ...
        query.append("from  (					");
        query.append("SELECT        ");
        query.append("         c.comp_no,c.plant,LEFT(b.wkor_date,6) yyyymm      ");
        query.append(" FROM TAPMSCHED a INNER JOIN TAWORKORDER b        ");
        query.append("  ON a.comp_no = b.comp_no AND a.pmsched_id = b.pmsched_id        ");
        query.append(" INNER JOIN TAPMLST c ON a.comp_no = c.comp_no AND a.pm_id = c.pm_id ");
        query.append(" INNER JOIN TAPMEQUIP d ON c.comp_no = d.comp_no AND c.pm_id = d.pm_id AND a.pmequip_id = d.pmequip_id ");
