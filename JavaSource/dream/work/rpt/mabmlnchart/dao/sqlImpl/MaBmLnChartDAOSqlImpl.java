package dream.work.rpt.mabmlnchart.dao.sqlImpl;
 ...
 */
public class MaBmLnChartDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaBmLnChartDAO
{
    /**
     * 라인고장분석 find grid
     * @author  kim21017
    ...
    public List findBmList(MaBmLnChartDTO maBmLnChartDTO)
    {
        ...
        query.append("		ISNULL((SELECT key_name FROM TALANG WHERE key_no='total2' AND key_type='LABEL' AND lang='"+lang+"'),'계') AS eqlocDesc");
        //계
        for (int i = 1; i <= 12; i++) {
			String month = i<10?"0"+i:i+"";
			query.append(",ISNULL(SUM(CASE SUBSTRING(x.start_date,1,6) WHEN '"+yyyy+month+"' THEN 1 END),0) AS CNT"+month);
    ...
    {        
        ...
        query.getAndQuery("SUBSTRING(x.start_date,1,4)", maBmLnChartDTO.getFilterYyyy().substring(0, 4));
