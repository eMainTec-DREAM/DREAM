 ...
 */
public class MaBdPointDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaBdPointDetailDAO
{
    /**
     * detail find
     * @author kim21017
    ...
    public MaBdPointDetailDTO findInsDetail(MaBdPointCommonDTO maBdPointCommonDTO, User loginUser)
    {
        ...
        query.append("		   ,y.start_date 													AS inspectDate		");
        ...
        query.append("		   ,dbo.SFACODE_TO_DESC(z.check_type,'CHECK_TYPE','SYS',x.comp_no,'"+loginUser.getLangId()+"') 							");
        query.append("		   																	AS checkType		");
        query.append("		   ,convert(nvarchar,z.check_min)+' / '+convert(nvarchar,z.check_basis_val)+' / '+convert(nvarchar,z.check_max)+		");
        query.append("			CASE WHEN z.uom IS NULL THEN '' ELSE ' . ('+z.uom+')' END		AS checkUom			");
        query.append("		   ,x.result_value 													AS resultValue		");
        query.append("		   ,dbo.SFACODE_TO_DESC(x.pm_point_rslt_status,'PM_POINT_RSLT_STATUS','SYS',x.comp_no,'"+loginUser.getLangId()+"') 		");
        ...
        query.append("		   ,dbo.SFACODE_TO_DESC(x.pm_point_rep_status,'PM_POINT_REP_STATUS','SYS',x.comp_no,'"+loginUser.getLangId()+"') 			");
    ...
    public MaBdPointDetailDTO findPInsDetail(MaBdPointCommonDTO maBdPointCommonDTO, User loginUser)
    {
        ...
        query.append("		   		AND a.emp_id = x.repair_by) 									AS repairName		");
        query.append("		   ,(SELECT a.param1 FROM TACDSYSD a 		                                            ");
        query.append("		     WHERE a.list_type = 'BM_TYPE' 		                                                ");
        query.append("		     AND a.key_no = 'BM_TYPE.'+(SELECT b.pm_type FROM TAWORKORDER b 					");
        query.append("                              WHERE b.wongpoint_id = x.wongpoint_id))		AS param1			");
    ...
    public int deleteWongPoint(String pmiType, String listKeyId, User user)
    {
    	...
    	query.append("DELETE x FROM TAWONGPOINT as x		");
    	query.append("WHERE 1=1													");
    	query.append("  AND x.comp_no = ?	    								");
    	query.append("  AND ISNULL(x.pm_rep_method_type,'-1') != 'DT'           ");	//조치하지 않은 항목
    ...
    public int insertWongDetail(String pmiType, String listKeyId, String pmPointId, String pointKeyId, String actualDate, String rsltValue, String remark, User user)
    {
    	...
    	query.append("    , pm_point_rep_status									");
    	query.append(") VALUES (												");
    	query.append("    	?   	, NEXT VALUE FOR SQAWONGPOINT_ID	, ?		");
    	query.append("    , ?													");
    	query.append("    , ? 		,?											");
    	query.append("    , ? 		,?											");
    	query.append("    , ? 		,?											");
	...
	public String findId(MaBdPointDetailDTO maBdPointDetailDTO, User loginUser) throws Exception {
    	...
    	query.append("SELECT 					");
    	query.append("    ISNULL(convert(varchar,x.woreq_id),'') 	 		");
    	query.append("    +','+  				");
    	query.append("    ISNULL(convert(varchar,x.woplan_wkor_id),'')   	");
