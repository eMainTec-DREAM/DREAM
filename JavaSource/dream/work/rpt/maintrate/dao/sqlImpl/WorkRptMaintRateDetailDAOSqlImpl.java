 ...
 */
public class WorkRptMaintRateDetailDAOSqlImpl  extends BaseJdbcDaoSupportSql implements WorkRptMaintRateDetailDAO
{
    ...
    public List findPartDetail(WorkRptMaintRateDetailDTO workRptMaintRateDetailDTO, User user) throws Exception
    {
        ...
        query.append("  , ISNULL(SUM(x.tot_amt),0)        AS USEAMT             ");
        query.append("  , COUNT(CASE WHEN x.wo_status = 'C' THEN 1 END) AS COMPLETECNT      ");
        query.append("  , ROUND(COUNT(CASE WHEN x.wo_status = 'C' THEN 1 END) / CONVERT(FLOAT, COUNT(1))*100,2)  AS COMPLETERATE         ");
        ...
        query.getOrderByQuery("x.comp_no", "(SELECT a.full_desc FROM TAEQLOC a "
    ...
    public List findTypeDetail(WorkRptMaintRateDetailDTO workRptMaintRateDetailDTO, User user) throws Exception
    {
        ...
        query.append("SELECT                                                    ");
        query.append("    ''                              AS SEQNO              ");
        query.append("  , dbo.SFACODETODESC(x.wo_type, 'WO_TYPE', 'SYS', x.comp_no) AS WOTYPEDESC     ");
        query.append("  , COUNT(1)                        AS ALLMAINTCNT        ");
        query.append("  , ISNULL(SUM(x.tot_amt),0)        AS USEAMT             ");
        query.append("  , COUNT(CASE WHEN x.wo_status = 'C' THEN 1 END) AS COMPLETECNT      ");
        query.append("  , ROUND(COUNT(CASE WHEN x.wo_status = 'C' THEN 1 END) / CONVERT(FLOAT, COUNT(1))*100,2)  AS COMPLETERATE         ");
        ...
        query.getOrderByQuery("x.comp_no", "dbo.SFACODETODESC(x.wo_type, 'WO_TYPE', 'SYS', x.comp_no)"
                , workRptMaintRateDetailDTO.getOrderBy(), workRptMaintRateDetailDTO.getDirection());
    ...
    public List findDayChart(WorkRptMaintRateDetailDTO workRptMaintRateDetailDTO, User user) throws Exception
    {
        ...
        query.getOrderByQuery("x.comp_no", "x.wkor_date", workRptMaintRateDetailDTO.getOrderBy(), workRptMaintRateDetailDTO.getDirection());
