package dream.work.rpt.mabmgwchart.dao.oraImpl;
 ...
 */
public class MaBmGwChartDAOOraImpl extends BaseJdbcDaoSupportOra implements MaBmGwChartDAO
{
    /**
     * 과별종합분석 find grid
     * @author  kim21017
    ...
    public List findBmList(MaBmGwChartDTO maBmGwChartDTO)
    {
        ...
        query.append("		NVL(MAX((SELECT key_name FROM TALANG WHERE key_no='total2' AND key_type='LABEL' AND lang='"+lang+"')),'계') AS deptDesc");
        //계
        for (int i = 1; i <= 12; i++) {
			String month = i<10?"0"+i:i+"";
			query.append(",NVL(SUM(DECODE(substr(x.start_date,0,6), '"+yyyy+month+"',1)),0) AS CNT"+month);
    ...
    {        
        ...
        query.getAndQuery("substr(x.start_date,0,4)", maBmGwChartDTO.getFilterYyyy().substring(0, 4));
      //담당부서
