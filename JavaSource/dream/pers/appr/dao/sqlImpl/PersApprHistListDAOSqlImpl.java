package dream.pers.appr.dao.sqlImpl;
 ...
 */
public class PersApprHistListDAOSqlImpl extends BaseJdbcDaoSupportSql implements PersApprHistListDAO
{
    ...
    { 
        ...
        query.append("    , dbo.SFACODE_TO_DESC(y.appr_type,'APPR_TYPE','SYS','','"+user.getLangId()+"' )       AS apprTypeDesc		");
        query.append("    , dbo.SFACODE_TO_DESC(y.appr_status,'APPR_STATUS','SYS','','"+user.getLangId()+"')	AS apprStatusDesc	");
        query.append("    , dbo.SFACODE_TO_DESC(x.APPRUSR_STATUS,'APPRUSR_STATUS','SYS','','"+user.getLangId()+"')    AS apprusrStatusDesc   ");
    ...
    {        
        ...
        query.append(" AND y.appr_status IN ('P', 'C', 'D')		");		// P:결재진행 C:결재완료 D:결재반려
        query.append(" AND x.APPRUSR_STATUS IN ('C', 'D')		");		// C:결재완료 D:결재반려
        // 회사
    ...
    public int deletePgmGuideList(String id, User user) throws Exception
    {
        ...
        query.append("DELETE FROM TAAPPRLIST		");
