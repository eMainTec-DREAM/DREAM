package dream.pers.priv.notice.dao.sqlImpl;
 ...
 */
public class PersPrivNoticeListDAOSqlImpl extends BaseJdbcDaoSupportSql implements PersPrivNoticeListDAO
{
    /**
     * grid find
     * @author  nhkim8548
    ...
    public List findList(MaMyInfoCommonDTO maMyInfoCommonDTO, User user)
    {
        ...
        query.append("     , dbo.SFACODE_TO_DESC(x.cdsysd_no, 'INIT_ALARM_LIST', 'SYS', '"+ user.getCompNo() +"', '"+ user.getLangId() +"')     AS alarmName        ");
        query.append("     , ISNULL(y.is_notice, 'N')                                                                                           AS isNotice         ");
        query.append("     , ISNULL(dbo.SFACODE_TO_DESC(y.is_notice, 'IS_USE', 'SYS', '"+ user.getCompNo() +"', '"+ user.getLangId() +"'), 'N') AS isNoticeDesc     ");
        ...
        query.getOrderByQuery("y.initalarmempset_id","y.initalarmempset_id", maMyInfoCommonDTO.getOrderBy(), maMyInfoCommonDTO.getDirection());
