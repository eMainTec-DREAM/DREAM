 ...
 */
public class AssetRptBdIntensityDetailListDAOOraImpl extends BaseJdbcDaoSupportOra implements AssetRptBdIntensityDetailListDAO
{
    /**
     * grid find
     * @author  youngjoo38
    ...
    public List findFreqChart(AssetRptBdIntensityDetailListForm assetRptBdIntensityDetailListForm, User loginUser)
    {
        ...
        query.append("SELECT                                                                                                                            ");
        query.append("      TO_CHAR(TO_DATE(a.tmonth,'yyyymm'),'yyyy-mm')                                                                  WKORDATE     ");
        query.append("    , ROUND(NVL(SUM(b.eqdn_time) / CASE WHEN SUM(b.loadTime) = 0 THEN NULL ELSE SUM(b.loadTime) END,0) * 100,2)      BDDURARATE   ");
        query.append("    , ROUND(NVL(SUM(b.eqdn_cnt) / CASE WHEN SUM(b.loadTime) = 0 THEN NULL ELSE SUM(b.loadTime) END,0) * 100,2)       BDFREQRATE   ");
        query.append("FROM TADAY a LEFT OUTER JOIN (                                                                                                    ");
        query.append("    SELECT x.equip_id, NVL(x.tday,y.tday) tday, x.loadTime, y.eqdn_time, y.eqdn_cnt FROM (                                        ");
        query.append("        SELECT                                                                                                                    ");
