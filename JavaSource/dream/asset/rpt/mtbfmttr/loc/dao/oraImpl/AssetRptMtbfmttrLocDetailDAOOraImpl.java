package dream.asset.rpt.mtbfmttr.loc.dao.oraImpl;
 ...
 */
public class AssetRptMtbfmttrLocDetailDAOOraImpl extends BaseJdbcDaoSupportOra implements AssetRptMtbfmttrLocDetailDAO
{
    /**
     * grid find
     * @author  ghlee
    ...
    public List findDetail(AssetRptMtbfmttrLocDetailDTO assetRptMtbfmttrLocDetailDTO, User loginUser)
    {
        ...
        query.append("AND x.eqloc_id IN(SELECT eqloc_id FROM TAEQLOC START WITH comp_no='"+loginUser.getCompNo()+"' AND eqloc_id='"+assetRptMtbfmttrLocDetailDTO.getEqLocId()+"' CONNECT BY PRIOR eqloc_id=p_eqloc_id)     ");
        ...
        query.append("    ,ROUND(NVL(SUM(b.work_time),0)/CASE SUM(b.bm_cnt) WHEN 0 THEN 1 ELSE SUM(b.bm_cnt) END, 2)    mttr            ");
        query.append("    ,CASE a.run_time WHEN 0 THEN 0 ELSE ROUND(NVL(SUM(b.eqdn_work_time),0)/a.run_time, 2) END     failureRate     ");
        query.append("FROM (                                                                                                            ");
        query.append("    SELECT                                                                                                        ");
        ...
        query.append("WHERE 1=1                                                                                                         ");
        query.append("AND b.eqloc_id IN(SELECT eqloc_id FROM TAEQLOC START WITH comp_no=a.comp_no AND eqloc_id=a.eqloc_id CONNECT BY PRIOR eqloc_id=p_eqloc_id)     ");
