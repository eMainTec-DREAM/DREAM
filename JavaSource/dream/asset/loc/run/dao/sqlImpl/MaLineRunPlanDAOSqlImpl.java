package dream.asset.loc.run.dao.sqlImpl;
 ...
 */
public class MaLineRunPlanDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaLineRunPlanDAO
{
    /**
     * grid find
     * @author kim21017
	...
	public String getColums(MaLineRunPlanDTO maLineRunPlanDTO, User user) {
        ...
        query.append("     , sum(isnull(x.ucnt,0)) OVER (partition by x.lnwrklist_id ORDER BY x.lnwrklist_id, x.wrk_date)	tot_ucnt		");
        query.append("     , sum(isnull(x.dtime,0) + isnull(x.ntime,0) + isnull(x.etime,0)) OVER (partition by x.lnwrklist_id ORDER BY x.lnwrklist_id, x.wrk_date)	tot_run_time	");
	...
	public String getOrderBy(MaLineRunPlanDTO maLineRunPlanDTO, User user) {
        ...
        query.getOrderByQuery("x.lnwrktime_id", "x.plant, x.wrk_date, x.eqloc_id", maLineRunPlanDTO.getOrderBy(), maLineRunPlanDTO.getDirection());
