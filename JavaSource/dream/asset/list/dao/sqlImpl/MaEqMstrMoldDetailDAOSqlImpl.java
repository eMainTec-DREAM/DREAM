 ...
 */
public class MaEqMstrMoldDetailDAOSqlImpl extends BaseJdbcDaoSupportSql implements MaEqMstrMoldDetailDAO 
{
    /**
     * detail find
     * @author kim21017
    ...
    public MaEqMstrDetailDTO findDetail(String equipId, String compNo, User user)
    {
        ...
        query.append("		dbo.SFACODETODESC(x.eq_status,'EQ_STATUS','SYS','') eqStatusDesc,	");
    ...
    public MaEqMstrMoldDetailDTO findMoldDetail(String equipId, User user)
    {
        ...
        query.append("    ,dbo.SFACODE_TO_DESC(eqmold_atype,'EQMOLD_ATYPE','USR',comp_no,'"+user.getLangId()+"') as eqMoldAtypeDesc ");
        query.append("    ,eqmold_btype      as eqMoldBtype          ");
        query.append("    ,dbo.SFACODE_TO_DESC(eqmold_btype,'EQMOLD_BTYPE','USR',comp_no,'"+user.getLangId()+"') as eqMoldBtypeDesc ");
        query.append("    ,eqmold_ctype      as eqMoldCtype          ");
        query.append("    ,dbo.SFACODE_TO_DESC(eqmold_ctype,'EQMOLD_CTYPE','USR',comp_no,'"+user.getLangId()+"') as eqMoldCtypeDesc ");
    ...
    public int insertEqHist(MaEqMstrDetailDTO maEqMstrDetailDTO)
    {
    	...
    	query.append("	)	VALUES											");
    	query.append("	(?,				NEXT VALUE FOR SQAEQALTHIST_ID, REPLACE(CONVERT(VARCHAR, GETDATE(), 120),'-',''),	");
    	query.append("	 ?,					?,					?,			");
    	query.append("	 ?,					?,					?,			");
    	query.append("	 ?,					?,					?,			");
    	query.append("	 ?,					?,					?			");
    ...
    public int insertEqAsmb(MaEqMstrDetailDTO maEqMstrDetailDTO)
    {
    	...
    	if(rtnValue>0){
        	...
        	query.append("	) SELECT 										");
        	query.append("	comp_no,		NEXT VALUE FOR SQAEQASMB_ID, ?,		");
        	...
        	rtnValue = this.getJdbcTemplate().update(query.toString(), getObject(objects1));
    	}
    ...
    public int updateDetail(MaEqMstrDetailDTO maEqMstrDetailDTO)
    {
    	...
    	query.append("	buy_amt			= NULLIF(?,''), is_law_eq		= ?,	");
    ...
    public int deletePmSched(MaEqMstrDetailDTO maEqMstrDetailDTO)
    {
    	...
    	query.append("WHERE  x.plan_date > CONVERT(VARCHAR, GETDATE(), 112)	");
    ...
    public List findReportEquipDetail(MaEqMstrDetailDTO maEqMstrDetailDTO) {
        ...
        query.append("		   WHERE a.comp_no = x.comp_no																			");
        query.append("			 AND a.equip_id = x.equip_id) assetNo,																");
        query.append("		dbo.SFACODETODESC(x.eq_status,'EQ_STATUS','SYS','') eqStatusDesc,										");
        query.append("		CONVERT(VARCHAR, CONVERT(DATE, x.setup_date), 23) 	setupDate,											");
        query.append("		 x.capacity									capacity,													");
        query.append("		 x.util_capa								utilCapa,													");
        query.append("		 x.buy_amt									buyAmt,														");
        query.append("		(SELECT description																						");
        ...
        query.append("		  WHERE comp_no = x.comp_no																				");
        query.append("		    AND dept_id = x.dept_id)				deptDesc,													");
        query.append("		REPLACE(CONVERT(VARCHAR(16), GETDATE(), 120),'-','') TODAY,												");
    ...
    public List findReportWoRepDetail(MaEqMstrDetailDTO maEqMstrDetailDTO) {
        ...
        query.append("SELECT CONVERT(VARCHAR, CONVERT(DATE, x.end_date), 23) repEndDate,");
        query.append("		(SELECT												");
        query.append("			(SELECT description								");
        query.append("				FROM TAEQASMB								");
    ...
    public List findReportWoOilDetail(MaEqMstrDetailDTO maEqMstrDetailDTO) {
        ...
        query.append("SELECT CONVERT(VARCHAR, CONVERT(DATE,x.end_date), 23) oilEndDate,");
        query.append("		(SELECT												");
        query.append("			(SELECT description								");
        query.append("				FROM TAEQASMB								");
        query.append("				WHERE eqasmb_id=a.eqasmb_id					");
	...
	public int copyDetail(MaEqMstrDetailDTO maEqMstrDetailDTO, String revisionHistId,String revisionStatus, User loginUser) 
	{
    	...
    	query.append("	 ,supplier 			,country_maker  	,ctctr_id 		");
    	query.append("	 ,eq_spec			,usage_emp			,weight	 		");
    	query.append("	 ,eq_size			,?			,CONVERT(VARCHAR,GETDATE(),112)+replace(CONVERT(VARCHAR,GETDATE(),108),':','')	");
    	query.append("   ,guaranty_date 	,?			,CONVERT(VARCHAR,GETDATE(),112)+replace(CONVERT(VARCHAR,GETDATE(),108),':','')	");
