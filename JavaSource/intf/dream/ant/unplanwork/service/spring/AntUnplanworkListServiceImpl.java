package intf.dream.ant.unplanwork.service.spring;
 ...
 * @since 1.0
 * 
 * @spring.bean id="antUnplanworkListServiceTarget"
 * @spring.txbn id="antUnplanworkListService"
 * @spring.property name="antUnplanworkListDAO" ref="antUnplanworkListDAO"
 ...
 */
public class AntUnplanworkListServiceImpl implements AntUnplanworkListService
{
    private AntUnplanworkListDAO antUnplanworkListDAO = null;
	...
	public AntUnplanworkListDAO getAntUnplanworkListDAO() {
		return antUnplanworkListDAO;
	}
	public void setAntUnplanworkListDAO(AntUnplanworkListDAO antUnplanworkListDAO) {
		this.antUnplanworkListDAO = antUnplanworkListDAO;
	}
	...
	public int saveWorkorder(List list)  throws Exception
	{
		...
		for(Object obj : list){
            ...
            if("Y".equals(CommonUtil.convertString(map.get("isNew")))){
            	wkorId = antUnplanworkListDAO.getNextSequence("SQAWKOR_ID");
            	//insert
            	antUnplanworkListDAO.insertWorkOrder(map, wkorId);
            	antUnplanworkListDAO.insertWorkOrderLog(map, wkorId);
            	...
            	//change Sequence
            	antUnplanworkListDAO.changeFileSeq(map, wkorId);
            	antUnplanworkListDAO.changeWoReqResWkorId(map, wkorId);
            	antUnplanworkListDAO.changeWoReqStatus(map, wkorId);
            }else{
            	wkorId = CommonUtil.convertString(map.get("wkorId"));
            	
            	//W/O 상태체크 후 상태가 업로드 대상이 아니면 Skip
            	if(!CommonUtil.isUseStringInArray(AntCommonValues.USED_WO_STATES, antUnplanworkListDAO.getWoStatusOfWorkOrder(map))){
            		continue;
            	}
            	
            	//update
            	antUnplanworkListDAO.updateWorkOrder(map);
            }
          ...
          //하위 insert
            if(equipList.size() != 0)
            {
            	for(Object equipObj : equipList)
                {
            		...
            		antUnplanworkListDAO.saveWoequip(equipMap,wkorId);
                }
            }
            ...
            if(craftList.size() != 0)
            {
                ...
                {
            		String woCraftId = antUnplanworkListDAO.getNextSequence("SQAWOCRAFT_ID");
            		
            		Map craftMap = (Map)craftObj;
            		antUnplanworkListDAO.saveWocraft(craftMap,wkorId,woCraftId);
            ...
            if(partsList.size() != 0)
            {
                ...
                {
            		String woPartId = antUnplanworkListDAO.getNextSequence("SQAWOPART_ID");
            		
            		Map partsMap = (Map)partsObj;
            		antUnplanworkListDAO.saveWoparts(partsMap,wkorId,woPartId);
            ...
            if(failList.size() != 0)
            {
            	for(Object failObj : failList)
                {
            		...
            		antUnplanworkListDAO.saveWofail(failMap,wkorId);
            		antUnplanworkListDAO.updateWorkOrderEqAsmgId(failMap,wkorId);
                }
            }
