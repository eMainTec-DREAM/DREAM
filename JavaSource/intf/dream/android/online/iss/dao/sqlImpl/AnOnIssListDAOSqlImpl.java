package intf.dream.android.online.iss.dao.sqlImpl;
 ...
 */
public class AnOnIssListDAOSqlImpl extends BaseJdbcDaoSupportSql implements AnOnIssListDAO
{
	public List findIssList(Map map) throws Exception
    {
    	...
    	query.append("		,ISNULL(c.description,'') +','+ ISNULL(c.pt_size,'')+','+ISNULL(c.model,'')+','+ISNULL(c.maker,'')	AS FULL_DESC		");
    	...
    	query.getLikeQuery("ISNULL(c.description,'') +','+ ISNULL(c.pt_size,'')+','+ISNULL(c.model,'')+','+ISNULL(c.maker,'')+ISNULL(c.part_no,'')", partDesc);
