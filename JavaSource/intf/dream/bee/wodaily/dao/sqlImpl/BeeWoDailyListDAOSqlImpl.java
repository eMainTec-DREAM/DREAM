package intf.dream.bee.wodaily.dao.sqlImpl;
 ...
 * @version $Id: $
 * @since   1.0
 * @spring.bean id="beeWoDailyListDAOTarget"
 * @spring.txbn id="beeWoDailyListDAO"
 * @spring.property name="dataSource" ref="dataSource"
 */
public class BeeWoDailyListDAOSqlImpl extends BaseJdbcDaoSupportSql implements BeeWoDailyListDAO
{
	public List findWoDailyList(BeeWoDailyCommonDTO beeWoDailyCommonDTO,Map map) throws Exception
    {
		String wodaylistId = CommonUtil.convertString(String.valueOf(map.get("wodaylistId")));
        ...
        query.append("        ,dbo.SFACODE_TO_DESC(a.wodaylist_status,'WODAYLIST_STATUS','SYS','','ko') 	AS STATUSDESC   ");
        ...
        if(!"".equals(wodaylistId))
        {
        	query.append(this.getWoDailyDetailColumn(beeWoDailyCommonDTO, map));
        }
        
        query.append("FROM   TAWODAYLIST a                                                                              ");
        query.append("WHERE 1=1        																					");
        query.append(this.getWoDailyWhere(beeWoDailyCommonDTO, map));

        query.getOrderByQuery("wo_date", "", "desc");

        return getJdbcTemplate().queryForList(query.toString(beeWoDailyCommonDTO.getIsLoadMaxCount(), beeWoDailyCommonDTO.getFirstRow()));
    } 
	
	private String getWoDailyDetailColumn(BeeWoDailyCommonDTO beeWoDailyCommonDTO, Map map)
	{
    	...
    	String startTime = CommonUtil.convertString(String.valueOf(map.get("startTime")));
    	String endTime = CommonUtil.convertString(String.valueOf(map.get("endTime")));
        ...
        query.getAndDateQuery("ISNULL(b.start_date,'000000')+ISNULL(b.start_time,'0000')", startFdateTime, startEdateTime);
        ...
        query.append("        ,(SELECT TOP 1 imgdata_id FROM (        													");
        ...
        query.append("        ,        																					");
        query.append("        (SELECT TOP 1 image_id FROM (        														");
	...
	private String getWoDailyWhere(BeeWoDailyCommonDTO beeWoDailyCommonDTO, Map map)
	{
	...
	public List findDailyUnPlanList(Map map) throws Exception
    {
        ...
        String startEdateTime = ("".equals(endDate)?"000000":endDate.replaceAll("-", ""))
                +("".equals(endTime)?"0000":endTime.replaceAll(":", ""));
        query.getAndDateQuery("ISNULL(a.start_date,'000000')+ISNULL(a.start_time,'0000')", startFdateTime, startEdateTime);
