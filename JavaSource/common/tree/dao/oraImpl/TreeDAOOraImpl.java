package common.tree.dao.oraImpl;
 ...
 */
public class TreeDAOOraImpl extends JdbcDaoSupport implements TreeDAO
{
    /**
    ...
    public List findT2TreeList(String parentNo, boolean folder)
    {
        ...
        //query.append("  AND  NVL(eq_status, 'NULL') <> 'A'              ");
    ...
    { 
        ...
        else if ((TreeAction.LOCATTREE).equals(treeType))
        {
            ...
            parents = QueryBuffer.toStringSingleArray(result);
        }
    ...
    public List findCalTreeList(String parentNo, boolean folder)
    {
        ...
        if("!".equals(parentNo))
        {
            if(folder)
            {
                ...
                query.append("FROM   (SELECT DISTINCT SUBSTR(a.worked_date, 1, 4) worked_date       ");
        ...
        else
        {
            ...
            {
                query.append("SELECT                                                                        ");
                query.append("       '['||SUBSTR(x.worked_date,1,4)||'.'||SUBSTR(x.worked_date,5,2)||']',   ");
                query.append("       x.worked_date,                                                         ");
                query.append("       x.worked_date description,                                             ");
                query.append("       x.worked_date dkeyNo                                                   ");
                query.append("FROM   (SELECT DISTINCT SUBSTR(a.worked_date, 1, 6) worked_date               ");
                query.append("        FROM   T2MOR_HDR a                                                    ");
                query.append("        WHERE  a.worked_date IS NOT NULL) x                                   ");
                query.append("WHERE  1=1                                                                    ");
                query.append("  AND  SUBSTR(x.worked_date, 1, 4) = '" + parentNo + "'                       ");
                query.append("  AND  x.worked_date IS NOT NULL                                              ");
                query.append("ORDER  BY x.worked_date DESC                                                  ");
            }
            else 
