...
public class ExcelWriter extends BaseWriter {
	...
	public void generate(String xml, String filePath, String fileName, HttpServletResponse resp){
		parser = new ExcelXmlParser();
		try {
			parser.setXML(xml);
			createExcel(filePath, fileName, resp);
			setColorProfile();
			headerPrint(parser);
	...
	private void createExcel(String filePath, String fileName, HttpServletResponse resp) throws IOException {
	...
	private void headerPrint(ExcelXmlParser parser) throws RowsExceededException, WriteException, IOException {
		...
		//헤더만 깨지는 현상이라서 헤더만 변경함.
		WritableFont.FontName mgodik = WritableFont.createFont("굴림");
		...
		if (parser.getWithoutHeader() == false) {
			for (int i = 0; i < cols.length; i++) {
				...
				for (int j = 0; j < cols[i].length; j++) {
					...
					WritableFont font = new WritableFont(mgodik, fontSize - 1, WritableFont.BOLD);
