CREATE FUNCTION [dbo].[SFAEQLOC_PALL]
(
...
RETURN(
    WITH DATATABLE AS ( 
        ...
        SELECT 
           ...
           , C.LVL+ 1
           , B.DESCRIPTION
        FROM TAEQLOC B INNER JOIN DATATABLE C  ON B.COMP_NO = C.COMP_NO AND B.EQLOC_ID = C.P_EQLOC_ID AND B.COMP_NO= @PCOMP_NO
        WHERE 1=1
    ) 
...
ALTER FUNCTION [dbo].[SFAEQLOC_PALL]
(
      @PCOMP_NO NVARCHAR(40)
    , @EQLOC_ID BIGINT
	, @DESCRIPTION NVARCHAR(40)
