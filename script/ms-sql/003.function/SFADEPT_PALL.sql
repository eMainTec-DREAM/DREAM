CREATE FUNCTION [dbo].[SFADEPT_PALL]
(
...
RETURN(
    WITH DATATABLE AS ( 
        ...
        SELECT 
           ...
           , B.P_DEPT_ID
           , C.LVL+ 1
        FROM TADEPT B INNER JOIN DATATABLE C  ON B.COMP_NO = C.COMP_NO AND B.DEPT_ID = C.P_DEPT_ID AND B.COMP_NO= @PCOMP_NO
        WHERE 1=1
    ) 
...
ALTER FUNCTION [dbo].[SFADEPT_PALL]
(
      @PCOMP_NO NVARCHAR(40)
    , @DEPT_ID BIGINT
	, @DESCRIPTION NVARCHAR(40)
