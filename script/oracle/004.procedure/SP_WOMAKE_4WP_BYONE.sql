CREATE OR REPLACE PROCEDURE SP_WOMAKE_4WP_BYONE(
...
BEGIN
   ...
   v_to_date  := to_char(sysdate + 31, 'yyyymmdd');
    ...
    -- delete all data
    delete from taworkorder4wp where comp_no = v_comp_no and wkor_id = v_wkor_id;
    delete from tawoequip4wp where comp_no = v_comp_no and wkor_id = v_wkor_id; 
    delete from tawoparts4wp where comp_no = v_comp_no and wkor_id = v_wkor_id;
    delete from tawocraft4wp where comp_no = v_comp_no and wkor_id = v_wkor_id;
    delete from tawopoint4wp where comp_no = v_comp_no and wkor_id = v_wkor_id;
